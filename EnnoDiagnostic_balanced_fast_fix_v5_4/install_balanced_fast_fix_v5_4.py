...
def assert_expected_lock_functions(path: Path, *, label: str) -> None:
    ...
    if changed:
        raise RuntimeError(
            f"{label}: la logique verrou locale n'est plus la base V5.3 attendue; "
            f"installation annulée pour ne pas écraser des changements. Détails={changed}"
        )


def main() -> None:
    parser = argparse.ArgumentParser(description="Installe EnnoDiagnostic V5.4 équilibré + rapide.")
    ...
    backup = repo / ".ennosmart_patch_backups" / (
        "balanced_fast_fix_v5_4_" + datetime.now().strftime("%Y%m%d_%H%M%S")
    )
    ...
    # Garde dure AVANT modification : ce paquet n'est applicable que sur la base
    # V5.3 fournie par l'utilisateur. On refuse d'écraser une logique verrou ayant changé.
    ...
    try:
        ...
        print("[OK] Correctif équilibré + rapide V5.4 installé.")
        print(f"[INFO] Rollback disponible : {backup}")

    except Exception:
        print("[ERREUR] Installation V5.4 interrompue. Restauration des 3 fichiers...", file=sys.stderr)
        for rel in PATCH_FILES:
            src = backup / rel
            dst = repo / rel
            if src.exists():
