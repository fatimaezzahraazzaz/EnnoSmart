...
export function ImprovementPdfComparator({
  ...
  useEffect(() => {
    ...
    Promise.allSettled([
      loadSide("original"),
      loadSide("proposed"),
    ]).then(([originalResult, proposedResult]) => {
      if (cancelled) return

      if (originalResult.status === "fulfilled") {
        setOriginalPreview(originalResult.value)
      } else {
        setOriginalPreview({
          ...EMPTY_PREVIEW,
          error:
      ...
      if (proposedResult.status === "fulfilled") {
        setProposedPreview(proposedResult.value)
      } else {
        setProposedPreview({
          ...EMPTY_PREVIEW,
          error:
            proposedResult.reason instanceof Error
              ? proposedResult.reason.message
