...
"""
EnnoSmart Agent 3 — comparatif PDF visuel V4.00.
...
def _materialize_document(document: Document, project_id: int) -> Path:
    ...
    if not raw:
        raise HTTPException(
            status_code=404,
            detail="Le fichier binaire du document source n'est plus disponible.",
        )
...
def _as_pdf(source: Path, project_id: int) -> Path:
    suffix = source.suffix.lower()
    if suffix in PDF_EXTENSIONS:
        return source.resolve()
    if suffix in OFFICE_TO_PDF_EXTENSIONS:
        return convert_office_to_pdf(source, project_id).resolve()
...
def _candidate_docx(
    ...
    manifest = {
        "version": "agent3-docx-patch-v400",
        "source": str(source),
...
def _base_proposed_pdf(
    ...
    if source.suffix.lower() in DOCX_EXTENSIONS:
        try:
            ...
            candidate_pdf = convert_office_to_pdf(
                candidate_docx, project_id
            ).resolve()
            return candidate_pdf, "faithful-docx", manifest
        except Exception as exc:
            return (
                original_pdf,
                "review-overlay",
...
def build_comparison_preview(
    ...
    *,
    ...
    original_pdf = _as_pdf(source, project_id)
