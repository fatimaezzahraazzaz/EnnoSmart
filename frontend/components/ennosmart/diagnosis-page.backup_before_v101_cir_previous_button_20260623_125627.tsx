...
export function DiagnosisPage() {
  ...
  const cirContinuityDiagnostic = useMemo(() => ({
    ...
    cir_memory_report: {
      ...(cirMemory || {}),
      ok: cirMemoryOk,
      has_previous_cir: cirMemoryHasPrevious,
      summary: cirMemorySummary,
      previous_cir_years_used: cirMemoryPreviousYears,
      previous_years: cirMemoryPreviousYears,
              ...
              {previousCirUploadReport && (
                <div className="rounded-lg border bg-success/5 border-success/20 p-4 space-y-2">
                  ...
                  <p className="text-xs text-muted-foreground">
                    {previousCirUploadReport.items_count || 0} passages ont été ajoutés à la mémoire CIR. Clique sur le bouton ci-dessous pour lancer la comparaison avec le CIR précédent.
                  </p>
                </div>
              )}
            </CardContent>
          ...
          <Card className="border-blue-200 bg-blue-50/70">
            <CardHeader>
              <CardTitle className="text-sm flex items-center gap-2">
                ...
                Lancer la comparaison avec le CIR précédent
              </CardTitle>
              <CardDescription className="text-xs">
                Après ajout ou remplacement du CIR précédent, lance ce bouton pour recalculer les passages de continuité entre le CIR N-1 et le dossier courant.
              </CardDescription>
            </CardHeader>
            <CardContent className="flex flex-wrap items-center gap-2">
              <Button
                ...
                className="bg-brand hover:bg-brand/90"
                onClick={runAgentOnly}
                disabled={running || (previousCirList.length === 0 && !previousCirUploadReport)}
              >
                {runningMode === "agent" ? (
                  <Loader2 className="size-4 mr-2 animate-spin" />
                ) : (
              ...
              {previousCirList.length === 0 && !previousCirUploadReport && (
                <p className="text-xs text-muted-foreground">
                  Ajoute d’abord un CIR précédent pour activer la comparaison.
                </p>
              )}
            </CardContent>
          </Card>

          <CirPreviousContinuityTab diagnostic={cirContinuityDiagnostic} />
