...
export function DiagnosisPage() {
  ...
  const cirContinuityDiagnostic = useMemo(() => {
    const hasPreviousCir = cirMemoryHasPrevious || previousCirAvailable
    const previousYears =
  ...
  // V102 - Le CIR précédent peut déjà exister en mémoire même si la comparaison
            ...
            <CardContent className="space-y-3">
              <div className="flex flex-wrap items-center gap-2">
                <Button
                  ...
                  className="bg-brand hover:bg-brand/90"
                  onClick={runAgentOnly}
                  disabled={running || !previousCirAvailable}
                >
                  {runningMode === "agent" ? (
                    <Loader2 className="size-4 mr-2 animate-spin" />
                  ) : (
