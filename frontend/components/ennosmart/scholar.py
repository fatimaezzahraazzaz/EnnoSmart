...
def _purge_old_project_articles_for_current_run(
    db: Session,
    project_id: int,
    keep_run_id: int,
) -> int:
    """
    Remet la table Article à l'état courant pour ce projet.

    On supprime les articles synchronisés depuis les anciens ScholarRun du même
    projet, afin d'éviter l'effet cumulatif :
      run 1 = 877 articles
      run 2 = 877 articles
      ancien bug affichage = 1754 articles

    Les ScholarRun restent conservés pour l'historique, mais les Article affichés
    correspondent au dernier run actif.
    """
    old_articles = (
        db.query(Article)
        .join(ScholarRun, Article.scholar_run_id == ScholarRun.id)
        .filter(
            ScholarRun.project_id == project_id,
            Article.scholar_run_id != keep_run_id,
        )
        .all()
    )
...
def run_scholar_from_selected_verrous(
    project_id: int,
    max_verrous: int = Query(8, ge=1, le=20),
    limit_per_query: int = Query(50, ge=1, le=100),
...
def sync_articles(
    ...
    synced_articles = sync_articles_from_scholar(db, run)

    # V10 : éviter l'accumulation des articles dans l'interface.
    # Après synchronisation du nouveau run, on supprime les articles des anciens runs.
    _purge_old_project_articles_for_current_run(
        db=db,
        project_id=project.id,
        keep_run_id=run.id,
    )
...
def fetch_scholar_selected_articles_fulltext(
    ...
    return resolve_and_extract_fulltext_for_selected_articles(
        db=db,
        project=project,
...
def extract_scholar_selected_fulltext_direct(
    ...
    return resolve_and_extract_fulltext_for_selected_articles(
        ...
        project=project,
        max_articles=effective_max_articles,
        # Le bouton « Relancer » relit les URLs déjà connues, pas les succès OCR.
        refresh_resolution=force,
        force_reextract=False,
    )
