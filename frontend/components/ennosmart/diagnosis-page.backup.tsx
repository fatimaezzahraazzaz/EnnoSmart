...
function MarkdownTableV93({ lines }: { lines: string[] }) {
          ...
          {rows.map((row, rowIndex) => (
            <tr key={rowIndex} className="hover:bg-muted/30">
              {header.map((_, cellIndex) => (
                ...
                >
                  <InlineMarkdownV93 text={row[cellIndex] || "—"} />
                </td>
              ))}
            </tr>
          ))}
...
function BackendSectionRendererV93({ text }: { text: string }) {
    ...
    return (
      <div className="space-y-4">
        {blocks.map((block, index) =>
          block.type === "table" ? (
            <MarkdownTableV93 key={index} lines={block.lines} />
          ) : (
    ...
    <Card className={toneClass}>
      ...
      <CardContent>
        <div className="rounded-xl border bg-white/80 p-4">
          {text?.trim() ? (
            <BackendSectionRendererV93 text={text} />
          ) : (
              ...
              {previousCirUploadReport && (
                <div className="rounded-lg border bg-success/5 border-success/20 p-4 space-y-2">
                  ...
                  <p className="text-xs text-muted-foreground">
                    {previousCirUploadReport.items_count || 0} passages ont été ajoutés à la mémoire CIR. Relance EnnoDiagnostic pour recalculer les passages de continuité.
                  </p>
                </div>
              )}
            </CardContent>
