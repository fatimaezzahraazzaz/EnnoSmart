...
CHAT_SERVICE_VERSION = "diagnostic_rag_chat_v7_7_0_passage_provenance"
...
def _metadata_text(meta: Mapping[str, Any], *keys: str) -> str:
    return " | ".join(
        _normalise(meta.get(key))
        for key in keys
        if meta.get(key) not in (None, "")
    )


def _passage_provenance(source: Mapping[str, Any]) -> Dict[str, Any]:
    """Classifie la provenance AU NIVEAU DU PASSAGE, sans hardcoding projet.

    Valeurs possibles :
    - project_current : passage attribuable au travail courant ;
    - external_literature : article / bibliographie / état de l'art externe ;
    - consultant_or_interviewer : question, demande ou remarque du consultant ;
    - administrative_or_cir_comment : remarque documentaire / CIR non technique ;
    - agent_analysis : sortie secondaire d'EnnoDiagnostic ;
    - ambiguous_or_mixed : provenance insuffisante ou passage mélangeant plusieurs voix.

    La fonction n'utilise aucun nom de projet, modèle, méthode, verrou ou fichier précis.
    Elle privilégie d'abord les métadonnées explicites déjà produites par le NLP,
    puis le titre de section, puis des indices linguistiques génériques.
    """
    meta = _source_metadata(source)
    text_raw = _clean(source.get("text"))
    text = _normalise(text_raw)
    section = _normalise(meta.get("section_title"))
    dtype = _normalise(meta.get("document_type"))
    ...
    if source_kind == "diagnostic_output":
        return {
            ...
            "confidence": "high",
            "reasons": ["source_kind=diagnostic_output"],
        }

    explicit_blob = _metadata_text(
        meta,
        "evidence_origin",
        "provenance_role",
        "content_origin",
    ...
    external_meta_markers = (
        "etat art", "etat de l art", "bibliograph", "literature", "litterature",
        "external", "externe", "article", "publication", "scientific paper",
        "related work", "state of the art",
    )
    project_meta_markers = (
        "project current", "current project", "projet courant", "preuve projet",
        "project evidence", "travaux projet", "project work", "real project",
    )
    consultant_meta_markers = (
        "consultant", "interviewer", "question consultant", "reviewer",
    )
    admin_meta_markers = (
        "administratif", "administrative", "cir comment", "frascati comment",
        "eligibilite documentaire", "dossier cir",
    )

    if any(marker in explicit_blob for marker in external_meta_markers):
        return {"origin": "external_literature", "confidence": "high", "reasons": ["metadata_external"]}
    if any(marker in explicit_blob for marker in consultant_meta_markers):
        return {"origin": "consultant_or_interviewer", "confidence": "high", "reasons": ["metadata_consultant"]}
    if any(marker in explicit_blob for marker in admin_meta_markers):
        return {"origin": "administrative_or_cir_comment", "confidence": "high", "reasons": ["metadata_administrative"]}
    if any(marker in explicit_blob for marker in project_meta_markers):
        return {"origin": "project_current", "confidence": "high", "reasons": ["metadata_project"]}

    # Types documentaires explicitement bibliographiques : signal fort.
    if dtype in {
        "etat art bibliographie", "etat_art_bibliographie", "bibliographie",
        "article scientifique", "scientific article", "publication scientifique",
    }:
        return {"origin": "external_literature", "confidence": "high", "reasons": ["document_type_external"]}

    external_section_markers = (
        "etat de l art", "etat art", "state of the art", "related work",
        "related works", "revue de litterature", "literature review",
        "bibliographie", "references bibliographiques", "references",
        "travaux existants", "scientific background",
    )
    project_section_markers = (
        "travaux realises", "travaux menes", "protocole experimental",
        "demarche experimentale", "experimentations", "resultats experimentaux",
        "resultats du projet", "verrous scientifiques ou techniques",
        "methodologie du projet", "tests realises", "essais realises",
    )
    admin_section_markers = (
        "eligibilite cir", "analyse cir", "frascati", "qualite du dossier",
        "documentation du dossier", "justificatif cir", "commentaires consultant",
    )

    if any(marker in section for marker in external_section_markers):
        return {"origin": "external_literature", "confidence": "high", "reasons": ["section_external"]}
    if any(marker in section for marker in admin_section_markers):
        return {"origin": "administrative_or_cir_comment", "confidence": "high", "reasons": ["section_administrative"]}

    strong_external_text_markers = (
        "cet article presente", "cet article propose", "ce papier presente",
        "ce papier propose", "l article presente", "l article propose",
        "les auteurs proposent", "les auteurs presentent", "selon les auteurs",
        "dans leur etude", "dans cette publication", "travaux de ",
        "article published", "this paper presents", "this paper proposes",
        "this article presents", "the authors propose", "the authors present",
        "according to the authors", "in their study", "published in",
        "doi ", "arxiv ", "et al ",
    )
    external_hits = [marker for marker in strong_external_text_markers if marker in text]

    # Questions / demandes : on exige des formes interrogatives génériques.
    consultant_question_markers = (
        "avez vous", "est ce que", "pouvez vous", "pourriez vous",
        "vous avez teste", "vous avez utilise", "vous avez mesure",
        "qu est ce que vous", "comment avez vous", "pourquoi avez vous",
        "peux tu", "peut on", "faut il",
    )
    question_hits = [marker for marker in consultant_question_markers if marker in text]
    has_question_punctuation = "?" in text_raw

    project_action_markers = (
        "nous avons teste", "nous avons utilise", "nous avons compare",
        "nous avons mesure", "nous avons obtenu", "nous avons developpe",
        "nous avons mis en place", "nous avons experimente", "nous avons evalue",
        "on a teste", "on a utilise", "on a compare", "on a mesure",
        "on a obtenu", "on a developpe", "on a mis en place", "on a experimente",
        "l equipe a teste", "l equipe a utilise", "l equipe a compare",
        "l equipe a mesure", "l equipe a developpe", "l equipe a experimente",
        "nos resultats", "notre protocole", "notre experimentation",
        "en interne nous", "nous en interne", "chez nous",
    )
    project_hits = [marker for marker in project_action_markers if marker in text]

    administrative_text_markers = (
        "non eligible au cir", "non eligibilite cir", "eligibilite au cir",
        "dossier cir", "document insuffisamment", "documentation insuffisante",
        "manque de documentation", "structuration du dossier", "preuve documentaire",
        "justificatif", "frascati",
    )
    admin_hits = [marker for marker in administrative_text_markers if marker in text]

    if external_hits:
        # Une section explicitement projet peut contenir une citation externe.
        # On conserve donc le niveau passage : la formulation article/auteurs gagne.
        return {
            "origin": "external_literature",
            "confidence": "high" if len(external_hits) >= 2 else "medium",
            "reasons": ["text_external"] + (["section_project_but_external_quote"] if any(m in section for m in project_section_markers) else []),
        }

    if question_hits and (has_question_punctuation or not project_hits):
        return {
            "origin": "consultant_or_interviewer",
            "confidence": "high" if has_question_punctuation else "medium",
            "reasons": ["question_form"],
        }

    if admin_hits and not project_hits:
        return {
            "origin": "administrative_or_cir_comment",
            "confidence": "medium",
            "reasons": ["administrative_text"],
        }

    if any(marker in section for marker in project_section_markers):
        return {
            "origin": "project_current",
            "confidence": "high" if project_hits else "medium",
            "reasons": ["section_project"] + (["project_action_language"] if project_hits else []),
        }
    ...
    if project_hits:
        return {
            "origin": "project_current",
            "confidence": "medium",
            "reasons": ["project_action_language"],
        }

    return {
        "origin": "ambiguous_or_mixed",
        "confidence": "low",
        "reasons": ["insufficient_passage_level_provenance"],
    }


def _statement_status(source: Mapping[str, Any], provenance: Optional[Mapping[str, Any]] = None) -> str:
    """Statut factuel du passage : réalisé, envisagé, question, externe, etc."""
    prov = dict(provenance or _passage_provenance(source))
    origin = _clean(prov.get("origin"))
    text_raw = _clean(source.get("text"))
    text = _normalise(text_raw)
    meta = _source_metadata(source)
    role = _normalise(meta.get("role") or meta.get("final_role"))

    if origin == "agent_analysis":
        return "agent_analysis"
    if origin == "external_literature":
        return "external_reported"
    if origin == "consultant_or_interviewer":
        return "question_or_request"
    if origin == "administrative_or_cir_comment":
        return "administrative_comment"

    planned_markers = (
        "nous allons", "on va", "il faudrait", "il faudra", "a faire",
        "a tester", "a evaluer", "a comparer", "est envisage", "sont envisages",
        "envisage", "envisagee", "prevu", "prevue", "perspective",
        "travaux futurs", "future work", "pourrait etre", "devrait etre",
        "objectif futur", "prochaine etape", "piste de travail",
    )
    realised_markers = (
        "nous avons teste", "nous avons utilise", "nous avons compare",
        "nous avons mesure", "nous avons obtenu", "nous avons developpe",
        "nous avons mis en place", "nous avons experimente", "nous avons evalue",
        "on a teste", "on a utilise", "on a compare", "on a mesure",
        "on a obtenu", "on a developpe", "on a experimente", "on a evalue",
        "a ete teste", "ont ete testes", "a ete evalue", "ont ete evalues",
        "resultats montrent", "nous observons", "nous avons observe",
        "nous avons constate", "mesure avec", "mesuree avec",
    )

    has_planned = any(marker in text for marker in planned_markers)
    has_realised = any(marker in text for marker in realised_markers)

    if has_planned and has_realised:
        return "mixed_realised_and_planned"
    if has_planned:
        return "planned_or_discussed"
    if has_realised:
        return "realised_or_observed"

    if origin == "project_current" and role == "resultat":
        return "realised_or_observed"

    return "unknown_or_ambiguous"


def _source_reliability_policy(source: Mapping[str, Any]) -> str:
    """Politique générique de prudence pour les transcriptions."""
    meta = _source_metadata(source)
    document = _normalise(meta.get("document"))
    source_path = _normalise(meta.get("source_path"))
    if "transcription" in document or "transcription" in source_path:
        return "transcription_attribute_numeric_claims_unless_corroborated"
    return "standard_documentary_source"


def _question_attribution_mode(question: str) -> str:
    q = _normalise(question)
    project_markers = (
        "l equipe", "equipe a", "travaux realises par", "travaux menes par",
        "experimentations realisees", "experimentations menees", "reellement realise",
        "reellement realisees", "a t elle realise", "ont ils realise",
        "travail courant", "travaux du projet",
    )
    external_markers = (
        "etat de l art", "litterature", "publication", "publications",
        "article externe", "articles externes", "methode externe", "methodes externes",
        "resultats externes", "provenant de l etat de l art", "travaux existants",
    )
    distinguish_markers = (
        "distingue", "distinguer", "separe", "separer", "difference entre",
        "ne presente jamais une publication", "travaux reellement realises",
    )

    asks_project = any(marker in q for marker in project_markers)
    asks_external = any(marker in q for marker in external_markers)
    asks_distinction = any(marker in q for marker in distinguish_markers)

    if asks_project and (asks_external or asks_distinction):
        return "compare_project_vs_external"
    if asks_project:
        return "project_realised_only"
    if asks_external:
...
def _evidence_nature(source: Mapping[str, Any]) -> str:
    ...
    if any(marker in text for marker in (
        "pourrait", "susceptible", "hypothese", "a confirmer",
        "envisage", "prevu", "piste de travail",
    )):
...
class DiagnosticRAGChatService:
                ...
                chunks.append(
                    {
                        ...
                        "metadata": {
                            ...
                            "role": _clean(meta.get("role")) or "document_brut",
                            "final_role": _clean(meta.get("final_role")) or _clean(meta.get("role")) or "document_brut",
                            # IMPORTANT : l'appartenance au corpus du projet ne signifie
                            # pas que le passage décrit un travail réalisé par l'équipe.
                            # Un même fichier peut mélanger projet, littérature et consultant.
                            "document_type": _clean(meta.get("document_type")) or "client_raw_document",
                            "source_policy": _clean(meta.get("source_policy")) or "current_project_document_scope_only",
                            ...
                            # Métadonnées de provenance amont conservées lorsqu'elles existent.
                            "evidence_origin": _clean(meta.get("evidence_origin")),
                            "provenance_role": _clean(meta.get("provenance_role")),
                            "content_origin": _clean(meta.get("content_origin")),
                            "source_role": _clean(meta.get("source_role")),
                            "evidence_scope": _clean(meta.get("evidence_scope")),
        ...
        if fulltext_files:
            for path in fulltext_files:
                ...
                base_meta = {
                    ...
                    "provenance_role": payload.get("provenance_role"),
                        ...
                        add_text(
                            ...
                            metadata={
                                **base_meta,
                                ...
                                "role": section.get("role") or base_meta.get("role"),
                                "final_role": section.get("final_role") or base_meta.get("final_role"),
                                "document_type": section.get("document_type") or base_meta.get("document_type"),
                                "source_policy": section.get("source_policy") or base_meta.get("source_policy"),
                                "evidence_origin": section.get("evidence_origin") or base_meta.get("evidence_origin"),
                                "provenance_role": section.get("provenance_role") or base_meta.get("provenance_role"),
                                "content_origin": section.get("content_origin") or base_meta.get("content_origin"),
                                "source_role": section.get("source_role") or base_meta.get("source_role"),
                                "evidence_scope": section.get("evidence_scope") or base_meta.get("evidence_scope"),
                                "speaker_role": section.get("speaker_role") or base_meta.get("speaker_role"),
    ...
    def _rank_and_dedupe_sources(
        ...
        *,
        ...
        for item, semantic_score in zip(prepared, semantic_scores):
            ...
            # V7.7 : reranking de provenance AU NIVEAU DU PASSAGE.
            # Une source brute du corpus n'est pas automatiquement une preuve
            # d'un travail réalisé par l'équipe : elle peut contenir état de l'art,
            # question consultant ou commentaire CIR.
            provenance = _passage_provenance(item)
            statement_status = _statement_status(item, provenance)
            attribution_mode = _question_attribution_mode(question)
            origin = _clean(provenance.get("origin"))

            if attribution_mode == "project_realised_only":
                if origin == "project_current":
                    score += 1.00
                elif origin == "external_literature":
                    score -= 1.40
                elif origin == "consultant_or_interviewer":
                    score -= 1.15
                elif origin == "administrative_or_cir_comment":
                    score -= 1.00

                if statement_status == "realised_or_observed":
                    score += 0.70
                elif statement_status in {"planned_or_discussed", "question_or_request"}:
                    score -= 0.85

            elif attribution_mode == "compare_project_vs_external":
                # Les deux familles restent visibles pour permettre la distinction,
                # mais consultant/admin ne doivent pas être promus comme expérimentation.
                if origin in {"project_current", "external_literature"}:
                    score += 0.30
                elif origin in {"consultant_or_interviewer", "administrative_or_cir_comment"}:
                    score -= 0.60

            elif attribution_mode == "external_literature":
                if origin == "external_literature":
                    score += 0.70
                elif origin == "project_current":
                    score -= 0.10
            ...
            enriched["_passage_provenance"] = provenance
            enriched["_statement_status"] = statement_status
            enriched["_attribution_mode"] = attribution_mode
    ...
    def _evidence_catalog(
        ...
        for source in sources[:max_sources]:
            ...
            provenance = dict(
                source.get("_passage_provenance")
                if isinstance(source.get("_passage_provenance"), Mapping)
                else _passage_provenance(source)
            )
            statement_status = _clean(
                source.get("_statement_status")
                or _statement_status(source, provenance)
            )
            reliability_policy = _source_reliability_policy(source)
            origin = _clean(provenance.get("origin")) or "ambiguous_or_mixed"

            if origin == "project_current" and statement_status == "realised_or_observed":
                attribution_policy = "may_support_team_realised_claim"
            elif origin == "external_literature":
                attribution_policy = "external_only_never_attribute_to_team"
            elif origin == "consultant_or_interviewer":
                attribution_policy = "question_or_comment_never_convert_to_fact"
            elif origin == "administrative_or_cir_comment":
                attribution_policy = "administrative_comment_not_technical_limit"
            elif statement_status == "planned_or_discussed":
                attribution_policy = "planned_or_discussed_never_present_as_realised"
            else:
                attribution_policy = "ambiguous_use_cautious_attribution"
            ...
            evidence.append(
                {
                    ...
                    # V7.7 : provenance et statut conservés jusqu'au prompt LLM.
                    "passage_origin": origin,
                    "passage_origin_confidence": _clean(provenance.get("confidence")) or "low",
                    "passage_origin_reasons": list(provenance.get("reasons") or []),
                    "statement_status": statement_status or "unknown_or_ambiguous",
                    "attribution_policy": attribution_policy,
                    "source_reliability_policy": reliability_policy,
...
2. `client_raw` et `nlp_rag` signifient uniquement que le passage provient directement
   du corpus documentaire. Cela NE signifie PAS que le passage décrit un travail réalisé
   par l'équipe. Un même document peut mélanger projet courant, littérature externe,
   questions du consultant et commentaires CIR.
3. Pour attribuer une action à l'équipe, respecte obligatoirement `passage_origin`,
   `statement_status` et `attribution_policy` de chaque preuve.
4. Une preuve `passage_origin=external_literature` peut être utilisée uniquement comme
   état de l'art / référence externe. Ne transforme JAMAIS ses méthodes, modèles,
   entraînements, métriques ou résultats en travaux réalisés par l'équipe.
5. Une preuve `passage_origin=consultant_or_interviewer` correspond à une question,
   demande ou remarque. Ne transforme JAMAIS cette question en fait projet.
6. Une preuve `statement_status=planned_or_discussed` ne peut jamais être décrite comme
   réalisée, testée ou obtenue. Utilise « envisagé », « discuté », « prévu » ou indique
   que la réalisation n'est pas établie.
7. Une preuve `passage_origin=administrative_or_cir_comment` concerne le dossier,
   l'éligibilité ou la documentation. Ne la présente jamais comme limite scientifique
   ou technique du projet.
8. Une preuve `ambiguous_or_mixed` ne suffit pas seule pour écrire « l'équipe a réalisé ».
   Utilise une formulation prudente (« le passage mentionne... ») ou indique que
   l'attribution n'est pas établie.
9. Si le mode est `project_realised_only`, seules les preuves `project_current` avec
   `realised_or_observed` peuvent soutenir directement « l'équipe a réalisé/testé/mesuré ».
   Si aucune preuve de ce type n'établit un point, écris que ce point n'est pas établi.
10. Si le mode est `compare_project_vs_external`, classe les PASSAGES et non les fichiers.
    Un document peut être MIXTE. Ne conclus jamais « ce document est entièrement état de l'art »
    ou « ce document prouve les travaux de l'équipe » si les preuves du document sont mixtes.
11. Si `source_reliability_policy=transcription_attribute_numeric_claims_unless_corroborated`,
    toute valeur numérique non corroborée doit être formulée comme :
    « la transcription mentionne ... » et non comme une vérité technique certaine.
12. `diagnostic_output` est une analyse secondaire. Ne l'utilise jamais à la place
    d'un passage documentaire disponible.
13. Chaque source citée doit contenir l'idée exacte de la phrase qu'elle soutient.
14. En portée stricte, aucune source d'un autre document n'est autorisée.
15. Ne transforme pas une hypothèse en résultat : « pourrait », « susceptible »,
    « envisagé » et « à confirmer » doivent conserver ce niveau d'incertitude.
16. N'affirme une amélioration, une supériorité ou une performance que si une
    preuve `resultat_mesure` l'établit explicitement ET que son attribution est correcte.
17. Pour une différence technique, cite d'abord les passages décrivant les principes,
    méthodes ou limites. N'utilise les résultats qu'en phrase séparée, si la question
    demande aussi l'effet expérimental.
18. Pour une question causale (« pourquoi »), donne uniquement les causes nécessaires.
    N'ajoute pas une comparaison si elle n'est pas indispensable.
19. Pour une question d'objectif, ne développe pas les résultats sauf demande explicite.
20. Si le document ne soutient pas un point, écris :
    « Ce point n'est pas établi dans les passages retrouvés. »
21. Ignore sommaires, listes de documents et bibliographies sauf demande explicite.
22. Cite immédiatement les affirmations avec [E1] ou [E1, E2].
23. Cite toutes les preuves réellement nécessaires pour soutenir la réponse, sans limite numérique fixe.
    En mode multi-document exhaustif, cite au moins une preuve réelle de CHAQUE document
    ayant une preuve pertinente, mais ne force jamais une attribution factuelle fausse
    uniquement pour citer ce document.
24. N'invente aucun identifiant de preuve.
25. Ne termine PAS par une ligne « Références ». Les cartes de sources sont affichées
    séparément par l'interface ; conserve seulement les citations inline [E1], [E2].
26. Si le type est `protocol_contrast`, cherche obligatoirement un passage qui énonce
...
27. Dans ce cas, réponds sous la forme :
...
- Pour `compare_project_vs_external`, distingue clairement :
  « Travaux réellement attribuables à l'équipe », « État de l'art / références externes »,
  puis « Ambigu / non établi » si nécessaire. Cette séparation se fait passage par passage,
  même à l'intérieur d'un même document.
...
28. Si le plan contient plusieurs unités, traite CHAQUE unité dans la réponse.
29. Utilise en priorité les `candidate_evidence_ids` associés à chaque unité, puis les autres preuves seulement en complément.
30. Une unité sans preuve candidate doit être signalée comme non établie ; n'invente jamais la réponse.
31. Ne remplace pas un chiffre, une classe, une limite ou une cause demandée par un résumé général si une preuve spécifique est disponible.
32. Pour une question demandant plusieurs résultats chiffrés, restitue tous les résultats explicitement présents dans les preuves retenues et pertinents pour les unités du plan, avec leur provenance correcte.
33. Si plusieurs résultats intermédiaires et un résultat final/principal sont présents,
...
34. Pour les limites, distingue strictement :
    - les limites techniques du travail courant ;
    - les limites de la littérature ou d'un article cité ;
    - les remarques documentaires/CIR.
    N'attribue au travail courant qu'une limite explicitement formulée comme telle dans
    ses propres résultats, discussion, conclusion ou perspectives.
35. Pour chaque unité du plan, utilise d'abord les preuves associées à cette unité.
...
36. Si le PLAN DE COUVERTURE EXHAUSTIVE MULTI-DOCUMENT contient des documents,
    crée obligatoirement une partie distincte pour CHACUN d'eux, dans l'ordre du plan.
37. Pour chaque document dont `candidate_found=true`, cite au moins une preuve appartenant
    à CE document, mais qualifie cette preuve selon son `passage_origin` et son `statement_status`.
38. Si `candidate_found=false` pour un document, conserve quand même sa partie et écris
    explicitement que l'information n'est pas établie dans les passages retrouvés.
39. Une demande « pour chaque/chacun/tous les documents » interdit d'omettre silencieusement
    un fichier, même si plusieurs autres documents contiennent des informations similaires.
40. Dans ce mode, ne termine pas par une synthèse globale qui efface les distinctions de provenance ;
    ...
    def answer(
        ...
        *,
        ...
        raw_answer = self.llm.generate(
            ...
            retries=1,
            json_mode=False,
            request_name="ennodiagnostic:rag_chat_v7_7_0",
        )
        ...
        if exhaustive_multi_document and multi_document_plan:
            ...
            if missing_keys:
                ...
                repaired_raw = self.llm.generate(
                    ...
                    retries=1,
                    json_mode=False,
                    request_name="ennodiagnostic:rag_chat_v7_7_0_multidoc_repair",
                )
        ...
        return {
            ...
            "retrieval": {
                ...
                "attribution_mode": _question_attribution_mode(clean_question),
                "evidence_provenance_summary": {
                    origin: sum(1 for item in evidence if _clean(item.get("passage_origin")) == origin)
                    for origin in sorted({_clean(item.get("passage_origin")) for item in evidence if _clean(item.get("passage_origin"))})
                },
