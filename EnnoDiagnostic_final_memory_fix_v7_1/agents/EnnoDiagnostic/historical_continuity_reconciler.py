...
VERSION = "historical_continuity_reconciler_v410_exact_memory_lock_local_probe"
    ...
    for hist_role, rows in support.items():
        for row in rows or []:
            ...
            if sim.score > best_support_score:
                ...
                best_support_role = str(hist_role)
...
def _recovered_gap_candidate(
    ...
    V410 : quand le recovery gate est franchi, la carte visible conserve le titre
    de la mémoire du même projet et son analyse historique, au lieu d'inventer
    « Continuité à confirmer — ... ». Les preuves cliquables restent exclusivement
    celles de N ; l'origine historique du libellé/analyse est explicitement tracée.
    """
    raw_sources = [
    ...
    if exact_memory_title and not _is_generic_historical_heading(exact_memory_title):
...
def _reconstruct_historical_families_with_llm_v300(llm, families):
    ...
    for row in data.get("families") or []:
        ...
        if _is_generic_historical_heading(representative.get("title")):
        ...
        representative.setdefault("historical_exact_title", representative.get("original_seed_title") or representative.get("title"))
        representative.setdefault("historical_exact_analysis", representative.get("text"))
...
def reconcile_historical_continuity(
    *,
    ...
    max_gap_families = max(0, int(os.getenv("ENNOSMART_HISTORICAL_GAP_MAX_FAMILIES", "12")))
