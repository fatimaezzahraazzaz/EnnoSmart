...
class ModernBERTAITextDetector:
    def __init__(
        ...
        self.max_tokens = max(64, _int_env("AI_DETECTOR_MAX_TOKENS", 512))
        ...
        # Nom conservé pour compatibilité avec l'ancien code, mais il contient
        # maintenant un prédicteur direct et non transformers.pipeline().
        self.pipeline = None
    ...
    def load_model(self):
        ...
        try:
            ...
            # L'import de `pipeline` charge des sous-modules optionnels
            # (vision/audio) et peut casser alors que le modèle texte fonctionne.
            # On charge donc uniquement les deux classes réellement nécessaires.
            try:
            ...
            use_cuda = self.device_name == "cuda" and torch.cuda.is_available()
            ...
            print(f"✅ AI detector chargé sur {device_label} (inférence directe, sans pipeline)")
            return self.pipeline
        except Exception as exc:
            self._model_load_error = f"Chargement du modèle IA impossible : {exc}"
            raise RuntimeError(self._model_load_error) from exc

    def _predict_direct(self, text: str) -> List[Dict[str, Any]]:
        """Équivalent minimal du pipeline text-classification."""
        if self.tokenizer is None or self.model is None or self._torch is None:
            raise RuntimeError("Le modèle IA n'est pas chargé.")

        tokenizer_limit = getattr(self.tokenizer, "model_max_length", self.max_tokens)
        try:
            tokenizer_limit = int(tokenizer_limit)
        except Exception:
            tokenizer_limit = self.max_tokens
        if tokenizer_limit <= 0 or tokenizer_limit > 100_000:
            tokenizer_limit = self.max_tokens
        max_length = max(64, min(self.max_tokens, tokenizer_limit))
        ...
        label = id2label.get(best_index, id2label.get(str(best_index), f"LABEL_{best_index}"))
        return [{"label": str(label), "score": float(probabilities[best_index].item())}]
    ...
    def analyze_text(self, text: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if not self._model_error_reported:
                print(f"⚠ Modèle IA indisponible, score heuristique conservé : {e}")
...
class EnnoAIDetectionService:
    ...
    def run(self, save: bool = True) -> Dict[str, Any]:
        ...
        result = {
            ...
            "ai_detection": report,
            "detector_runtime": {
                "model_available": self.detector.pipeline is not None and not self.detector._model_load_error,
                "fallback_heuristic_used": bool(self.detector._model_load_error),
                "inference_mode": "direct_auto_model_without_pipeline",
            },
            "summary": {
