...
_DEFAULTS: Dict[str, SectionContextConfig] = {
    ...
    "resultats_metriques": SectionContextConfig(
        ...
        max_chars_per_evidence=700,
        display_mode="numbered_items",
        # Un dossier peut ne contenir qu'un seul résultat réellement observé.
        # Imposer deux paragraphes poussait le rédacteur à transformer une cible
        # ou une planification en résultat acquis.
        min_items=1,
        max_items=10,
    ),
...
def get_section_config(section_key: str) -> SectionContextConfig:
    ...
    return SectionContextConfig(
        ...
        min_items=_env_int(
            f"ENNOSMART_DIAG_{env_key}_MIN_ITEMS",
            base.min_items,
            1,
        ),
        max_items=_env_int(
            f"ENNOSMART_DIAG_{env_key}_MAX_ITEMS",
            base.max_items,
            1,
        ),
