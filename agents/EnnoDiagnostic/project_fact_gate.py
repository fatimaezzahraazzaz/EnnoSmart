...
"""Gate générique des faits projet EnnoDiagnostic — V5.7 équilibrée.
...
_EXECUTED_ACTION_RE = re.compile(
    ...
    r"implement\w*|configur\w*|appliqu\w*|execut\w*|analys\w*|realis\w*|"
    r"effectu\w*|utilis\w*|modelis\w*|simul\w*|calcul\w*)\b|"
    r"\b(?:tested|evaluated|measured|compared|generated|implemented|trained|performed)\b",
    re.I,
)

# Les rapports techniques mélangent souvent expérience et résultat dans un
# même passage. Une action explicite dans le texte ORIGINAL peut compléter le
# rôle NLP ; les mots du contexte voisin ne doivent pas changer ce rôle.
_EXPLICIT_EXECUTED_METHOD_RE = re.compile(
    r"\b(?:nous avons|l['’]?equipe a)\s+(?:(?:ensuite|ainsi|egalement|aussi)\s+)?"
    r"(?:test\w*|evalu\w*|mesur\w*|compar\w*|entrain\w*|simul\w*|"
    r"experiment\w*|calcul\w*|analys\w*|implemente\w*)\b|"
    r"\b(?:we|i)\s+(?:have\s+)?(?:(?:then|also|successfully)\s+)?"
    r"(?:tested|evaluated|measured|compared|trained|simulated|experimented|"
    r"performed|conducted|implemented)\b|"
    r"\b(?:tests?|experiments?|simulations?|measurements?)\s+"
    r"(?:was|were|have been)\s+(?:performed|conducted|compared)\b|"
    r"\b(?:etude|analyse|experimentation)\s+a consiste a\s+"
    r"(?:etudier|tester|comparer|evaluer|mesurer|simuler|analyser)\b",
    re.I,
)

_OBSERVED_OUTCOME_RE = re.compile(
    r"\b(?:we|i)\s+(?:found|observed|measured|obtained)\b|"
    ...
    r"\b(?:resultats?|frequences?|valeurs?|mesures?)\s+(?:obtenu(?:e?s?)?|mesure(?:e?s?)?)\b|"
    r"\b(?:les resultats?|les frequences?|les valeurs?)\s+(?:sont|etaient)\s+les suivant(?:e?s?)\b|"
    r"\b(?:aucun|aucune|certains?|certaines?)\s+resultats?\b.{0,80}\b(?:correct|coherent|satisfaisant)\w*\b|"
    r"\b(?:a ete|ont ete)\s+(?:recale\w*|identifie\w*|retrouve\w*|observe\w*)\b",
    re.I,
)
...
_PARAMETER_RE = re.compile(
    ...
    r"frequence|raideur|rigidite|longueur|epaisseur|diametre|masse|densite|"
    r"module d['’ ]?young|coefficient de poisson|maillage|taille de maille|"
...
_NON_NUMERIC_CONSTRAINT_RE = re.compile(
    ...
    r"gpu|cpu|latence\w*|enveloppe technique|restriction\w*|obligation\w*|"
    r"frequence\w*|raideur\w*|rigidite\w*|longueur\w*|maillage\w*|"
    r"condition\w* limite\w*|liaison\w*|configuration\w*)\b",
    re.I,
)
...
_ADMIN_OR_TRACE_RE = re.compile(
    ...
    r"indiquez le niveau de confidentialite|niveau de diffusion|"
    r"temps disponible|temps (?:est|reste) limite|documentation brute|"
    r"documentation initiale|documents? initialement fournis?|"
    r"eligibilite(?: au)? cir|ineligibilite|estimation (?:prealable )?d['’]?ineligibilite|"
    r"preparer (?:un|le) dossier|preparer (?:un|le) document structure|"
    r"validation consultant|consultant cir|dossier cir)\b",
    re.I,
)
...
_RESULT_DOCUMENT_TYPE_RE = re.compile(
    ...
    r"experimental_results?|test_results?|rapport_test|rapport_essai|"
    r"rapport_experimental|extraction_modale|etude_convergence)\b",
    re.I,
)

_PROJECT_DOCUMENT_TYPE_RE = re.compile(
    r"\b(?:concept_projet|methodologie_protocole|resultats?_mesures?|"
    r"rapport_essai|rapport_test|rapport_experimental|compte_rendu|"
    r"specification_projet|project_core|extraction_modale|etude_convergence)\b",
    re.I,
)

_DOCUMENTED_METHOD_RE = re.compile(
    r"\b(?:essais?|tests?|mesures?|simulations?|modelisations?|calculs?|analyses?)\b"
    r".{0,100}\b(?:realise\w*|effectue\w*|applique\w*|utilise\w*|teste\w*|compare\w*)\b|"
    r"\b(?:materiel|equipement|configuration|protocole)\s+(?:utilise|teste|mesure)\w*\b|"
    r"\b(?:une etude|une analyse|une simulation|un calcul)\s+a ete\s+(?:realisee|effectuee|menee)\b",
    re.I,
)

_RESULT_SECTION_RE = re.compile(
    r"\b(?:resultats? obtenus?|resultats? des essais|observations?|"
    r"frequences? propres obtenues?|analyse des resultats?|erreurs? relatives?|recalage)\b",
    re.I,
)
...
def _project_direct_signal(source: Mapping[str, Any]) -> bool:
    ...
    if _PROJECT_WORK_SECTION_RE.search(section):
        return True
    # Les rapports d'essais et de simulation du corpus diagnostic courant sont
    # des productions directes du projet même lorsqu'ils emploient le passif
    # ("des essais ont été réalisés") et ne répètent pas "nous avons" dans
    # chaque ligne ou cellule. Cette ancre documentaire reste interdite aux
    # transcriptions et à la littérature, filtrées séparément en amont.
    declared_current = _truthy(_first(
        source,
        "current_project_evidence", "diagnostic_corpus_selected",
        "declared_raw_document",
    ))
    declared_corpus = _norm(_first(source, "declared_corpus"))
    current_content = _norm(_first(source, "content_origin", "source_type"))
    if (
        not is_transcription_source(source)
        and (declared_current or declared_corpus.startswith("diagnostic"))
        and (
            _project_document_signal(source)
            or current_content in {
                "raw_client_document", "nlp_result_current_project",
                "diagnostic_current", "document_brut",
            }
        )
    ):
        return True
...
def has_explicit_executed_method(source: Mapping[str, Any]) -> bool:
    """Preuve textuelle d'une action, jamais une inférence depuis le rôle NLP."""
    document_type = _norm(_first(source, "document_type", "document_category"))
    if document_type in {"etat_art_bibliographie", "scientific_article", "external_literature"}:
        return False
    primary = _norm(_first(source, "source_text_original", "text", "excerpt"))
    return bool(_EXPLICIT_EXECUTED_METHOD_RE.search(primary))


def method_evidence_candidates(pack: Mapping[str, Any]) -> List[Mapping[str, Any]]:
    """Inclut les expériences décrites dans les résultats, sans changer le NLP.

    Le filtrage de provenance est encore obligatoire pour chaque candidat.
    Seule une action explicite, jamais le seul rôle, ouvre ce chemin secondaire.
    """
    methods = [item for item in (pack.get("methodes_locales") or []) if isinstance(item, Mapping)]
    for key in ("resultats_locaux", "contributions_locales"):
        methods.extend(
            item for item in (pack.get(key) or [])
            if isinstance(item, Mapping) and has_explicit_executed_method(item)
        )
    return methods
...
def gate_project_fact(source: Mapping[str, Any], section_key: str) -> GateDecision:
    ...
    # Un verrou FINAL déjà validé par la chaîne dédiée peut alimenter la
    # synthèse stratégique sans être redétecté ici. Ce marqueur est créé
    # uniquement par le présentateur à partir de ``llm_reformulated_verrous`` ;
    # il ne permet jamais de créer/modifier un verrou.
    if key == "synthese_strategique" and _truthy(_first(source, "final_lock_context")):
        if is_noise_or_interview(source):
            return GateDecision(False, "final_lock_context_noise", 0.99)
        return GateDecision(True, "validated_final_lock_context", 0.99)
    ...
    if key == "objectif_global":
        ...
        # Un objectif peut provenir d'une transcription projet et perdre le pronom
        # "nous" lors du fenêtrage NLP (ex. "C'est le premier objectif. Et le
        # deuxième objectif..."). On accepte ce cas uniquement lorsque le
        # passage est explicitement marqué comme preuve du projet courant, vient
        # d'un document projet et contient un marqueur d'objectif sans attribution
        # à des auteurs externes. Cela évite de réouvrir la porte à la littérature.
        explicit_project_objective = bool(
            _truthy(_first(source, "current_project_evidence", "declared_raw_document"))
            and project_doc
            and re.search(
                r"\b(?:premier objectif|deuxieme objectif|objectif du projet|"
                r"c['’]?est (?:le|notre) .*?objectif|objectif-ci)\b",
                body,
                flags=re.I,
            )
            and not _EXTERNAL_ATTRIBUTION_RE.search(body)
        )
        ...
        if not (project_direct or explicit_project_objective):
            return GateDecision(False, "objective_actor_not_project_anchored", 0.98)
        return GateDecision(
            True,
            "project_objective_context" if explicit_project_objective and not project_direct else "project_objective",
            0.95,
        )

    if key == "demarche_detectee":
        explicit_method = has_explicit_executed_method(source)
        if roles and not (roles & {"methode", "parametre"}) and not explicit_method:
        ...
        documented_execution = bool(
            project_doc and _DOCUMENTED_METHOD_RE.search(f"{section} {body}")
        )
        if not (explicit_method or _EXECUTED_ACTION_RE.search(body) or executed or documented_execution):
    ...
    if key == "resultats_metriques":
        ...
        documented_result = bool(
            result_doc
            and (
                explicit_observation
                or _RESULT_SECTION_RE.search(f"{section} {body}")
                or (_RESULT_RE.search(body) and bool(_NUMERIC_RE.search(body)))
            )
        )
