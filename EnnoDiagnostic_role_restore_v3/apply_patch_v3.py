...
PATCH_VERSION = "v3_restore_nlp_role_authority"
...
def patch_presenter(text: str) -> str:
    ...
    old = '''        # V2 — défense en profondeur : pour les faits du projet courant,\n        # `ambiguous_current_dossier` est bloqué au même titre que la littérature.\n        # Cela n'affecte pas les cartes/verrous, seulement les preuves narratives aval.\n        if purpose in {"demarche_detectee", "resultats_metriques"} and origin != "project_direct":\n            continue\n        if (\n            purpose == "justification_frascati"\n            and stage in {\n                "uncertainty", "hypothesis", "hypothesis_component", "experiment",\n                "result", "learning", "systematicity", "transferability",\n                "creativity", "novelty",\n            }\n            and origin != "project_direct"\n        ):\n            continue'''
    new = '''        # V3 — le rôle NLP du corpus courant peut confirmer une démarche/résultat\n        # même si l'acteur n'est pas formulé explicitement dans la phrase. Les\n        # preuves externes restent bloquées par evidence_provenance.py.\n        if purpose in {"demarche_detectee", "resultats_metriques", "parametres_contraintes"}:\n            if not is_trusted_current_project_evidence(proof, purpose):\n                continue\n        if purpose == "justification_frascati":\n            if stage in {"experiment", "systematicity", "creativity"}:\n                allowed = is_trusted_current_project_evidence(proof, "demarche_detectee")\n            elif stage in {"result", "learning", "transferability"}:\n                allowed = is_trusted_current_project_evidence(proof, "resultats_metriques")\n            elif stage in {"uncertainty", "novelty"}:\n                allowed = is_trusted_current_project_evidence(proof, "verrou")\n            else:\n                allowed = origin == "project_direct"\n            if not allowed:\n                continue'''
    text = replace_once(text, old, new, "presenter:official role-aware provenance")
    ...
    """V3 : bloque l'externe, pas un rôle NLP courant déjà qualifié."""
    guarded_sections = {
...
'''
    ...
    old_contract = '''- V2 STRICTE : pour affirmer un objectif, une action, une démarche, un résultat, un apprentissage ou un paramètre du projet courant, la preuve doit avoir `evidence_origin=project_direct`.\n- `external_literature` reste disponible pour l'état de l'art/verrou mais ne prouve jamais un travail réalisé par le projet.\n- `ambiguous_current_dossier` signifie précisément que l'acteur n'est pas établi : interdiction de le transformer en fait du projet.\n- `actor_scope=external_authors` ou `unknown` ne doit jamais être reformulé comme « l'équipe », « nous » ou « le projet ».'''
    new_contract = '''- V3 : le rôle NLP est l'autorité sémantique pour `methode`, `resultat` et `parametre`; le presenter ne doit pas refaire cette classification par regex.\n- Une preuve `ambiguous_current_dossier` peut servir uniquement à la section technique correspondant à son rôle NLP si elle appartient au corpus courant et ne présente aucun signal d'état de l'art/auteur tiers.\n- `external_literature` reste toujours interdite comme travail réalisé par le projet.\n- Une cible/planification reste interdite comme résultat acquis. Objectif et synthèse restent `project_direct` uniquement.'''
    text = replace_once(text, old_contract, new_contract, "presenter:v3 prompt contract")
...
def latest_backup(root: Path) -> Path:
    ...
    candidates = sorted([p for p in backup_root.glob("role_restore_v3_*") if p.is_dir()], reverse=True)
    if not candidates:
        raise RuntimeError("Aucune sauvegarde role_restore_v3 trouvée.")
    return candidates[0]


def rollback(root: Path, backup: Path | None) -> None:
    backup = backup or latest_backup(root)
    print(f"[ROLLBACK V3] source={backup}")
    for rel in TARGETS:
        src = backup / rel
        dst = root / rel
        if src.exists():
...
def apply(root: Path, package_dir: Path) -> None:
    ...
    if "ENNODIAG_ROLE_AUTHORITY_V3" in originals[PRESENTER]:
        compile_targets(root)
        run_module_self_test(root / PROVENANCE)
        print("[OK] V3 déjà installée. Aucun fichier modifié.")
        return
    ...
    if "V2 stricte" not in originals[PROVENANCE]:
        raise RuntimeError("evidence_provenance.py V2 attendu avant V3.")
    ...
    backup = root / ".ennosmart_patch_backups" / f"role_restore_v3_{stamp}"
    ...
    try:
        ...
        print("[ERROR] échec V3 ; restauration automatique...")
    ...
    for rel in TARGETS:
        ...
        diff_parts.extend(difflib.unified_diff(
            ...
            fromfile=f"a/{rel} (avant V3)",
            tofile=f"b/{rel} (V3)",
        ))
    diff_path = root / "EnnoDiagnostic_role_restore_v3.diff"
    diff_path.write_text("".join(diff_parts), encoding="utf-8")

    print("\n[OK] Correctif V3 appliqué et compilé.")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="EnnoDiagnostic V3 — restaure l'autorité des rôles NLP")
...
if __name__ == "__main__":
    try:
        ...
        print(f"\n[ECHEC V3] {type(exc).__name__}: {exc}", file=sys.stderr)
        raise
