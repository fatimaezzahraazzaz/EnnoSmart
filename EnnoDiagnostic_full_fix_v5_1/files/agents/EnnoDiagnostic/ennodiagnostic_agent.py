# -*- coding: utf-8 -*-
# ENNODIAG_FULL_FIX_V5_20260829 — provenance, missing-lock recovery, no downstream lock creation
...
class EnnoDiagnosticAgent:
    ...
    def _load_current_nlp_evidence_sections(self) -> Dict[str, List[Dict[str, Any]]]:
        ...
        external_phrase_re = re.compile(
            r"\b(?:dans (?:le|l['’]) papier|the paper|les auteurs|the authors|selon (?:les auteurs|l['’]etude|l['’]article)|"
            r"l['’]etude a (?:inclus|porte)|analyse de \d+ (?:papers?|articles?|publications?)|"
            ...
            r"les resultats montrent que .*\bcouramment utilises?\b|travaux anterieurs|et al\\.)\b",
            re.I,
        )
        ...
        state_art_rows: List[Tuple[str, set[str]]] = []
        for ref in pack.get("etat_art_local") or []:
            if not isinstance(ref, dict):
                continue
            blob = ntext(item_blob(ref))
            tokens = {tok for tok in blob.split() if len(tok) >= 4}
            if tokens:
                state_art_rows.append((clean_text(ref.get("document")).lower(), tokens))
        ...
        def is_reference_like(item: Dict[str, Any]) -> Tuple[bool, List[str]]:
            ...
            tokens = {tok for tok in blob.split() if len(tok) >= 4}
            document = clean_text(item.get("document")).lower()
            if tokens:
                best = 0.0
                for ref_document, ref_tokens in state_art_rows:
                    if document and ref_document and document != ref_document:
                        continue
                    overlap = len(tokens & ref_tokens) / max(1, min(len(tokens), len(ref_tokens)))
                    best = max(best, overlap)
                if best >= 0.52:
                    reasons.append(f"high_overlap_with_state_of_art:{best:.2f}")
    ...
    def _load_recovered_missing_lock_candidates(
        ...
        *,
        ...
        """Récupère uniquement une contrainte forte oubliée par les groupes NLP.

        Ce chemin n'utilise aucun vocabulaire de projet. Il exige simultanément :
        une formulation de contrainte forte, un ancrage explicite de l'équipe/projet,
        l'absence de signal bibliographique et un passage du corpus courant.
        Les passages d'une même section documentaire sont fusionnés en un seul
        candidat à validation humaine.
        """
        ...
        strong_constraint_re = re.compile(
        ...
        noise_re = re.compile(
            r"\b(?:tableau|table des matieres|diffusion|masque des diapositives|code couleur|"
            r"titre du verrou|est ce qu on peut commencer)\b",
            re.I,
        )

        existing_text = " ".join(source_text(src) for src in existing_lock_sources if isinstance(src, dict))
        existing_tokens = {tok for tok in _diag_norm(existing_text).split() if len(tok) >= 5}
        ...
        for raw in catalog:
            ...
            if not strong_constraint_re.search(normalized) or not project_anchor_re.search(normalized):
                continue
            if external_re.search(normalized) or noise_re.search(normalized):
                continue
            ...
            by_section.setdefault(signature, []).append(raw)

        recovered: List[Dict[str, Any]] = []
        for signature, rows in by_section.items():
            # Conserver au plus les trois passages les plus informatifs de la section.
            rows = sorted(rows, key=lambda row: len(clean_text(row.get("analysis_text") or row.get("text"))), reverse=True)[:3]
            combined = clean_text(" ".join(clean_text(row.get("analysis_text") or row.get("text"), 1800) for row in rows), 4200)
            tokens = {tok for tok in _diag_norm(combined).split() if len(tok) >= 5}
            # Ne pas supprimer un candidat parce qu'un groupe NLP trop large partage
            # du vocabulaire : c'est précisément le cas que cette récupération corrige.
            primary = rows[0]
            sentence = next((
                clean_text(part, 500)
                for part in re.split(r"(?<=[.!?])\s+|[\r\n]+", combined)
                if strong_constraint_re.search(_diag_norm(part))
            ), clean_text(primary.get("section_title"), 500) or "Contrainte forte à qualifier")
            group_id = "recovered_constraint_" + hashlib.sha1(signature.encode("utf-8", errors="ignore")).hexdigest()[:14]
            meta = {
                "role": "verrou",
                ...
                "supporting_passages": [dict(row) for row in rows],
            }
            recovered.append({
                "text": combined,
        ...
        if recovered:
            print(f"[EnnoDiagnostic][MISSING_LOCK_RECOVERY] candidats={len(recovered)}", flush=True)
        return recovered
...
""".strip()
    ...
    def generate_diagnostic(self, save: bool = True) -> Dict[str, Any]:
        ...
        # V5 : la composition finale reste celle des groupes NLP + éventuels
        # candidats de récupération stricts. La consolidation d'axes aval est
        # désactivée par défaut car elle ne doit jamais créer/supprimer des verrous.
        atomic_verrous = list(llm_reformulated_verrous)
        scientific_axis_report: Dict[str, Any] = {
            "ok": False,
            "disabled": True,
            "policy": "nlp_authority_no_downstream_lock_creation",
            "atomic_verrous": atomic_verrous,
            "scientific_axes": [],
            "axis_count": 0,
        }
        use_axis = str(os.getenv("ENNOSMART_DIAG_USE_SCIENTIFIC_AXIS_CONSOLIDATION", "0")).strip().lower() in {
            "1", "true", "yes", "oui", "on"
        }
        if use_axis:
            try:
                try:
                    from agents.EnnoDiagnostic.scientific_axis_synthesizer import synthesize_scientific_axes
                except Exception:
                ...
                # Audit seulement : même activée, la consolidation ne remplace plus
                # la liste officielle affichée.
                scientific_axis_report["audit_only"] = True
            except Exception as exc:
                scientific_axis_report = {**scientific_axis_report, "ok": False, "error": str(exc)}
