...
def _build_historical_families(
...
def _write_report(output_dir: Optional[str | Path], report: Mapping[str, Any]) -> Optional[str]:
    if output_dir in (None, ""):
        return None
    try:
        path = Path(output_dir) / "historical_continuity_report_v200.json"
    ...
    *,
    ...
    try:
        ...
        report = {
            ...
            "current_before_count": len(current),
    ...
    for fid, cids in family_groups.items():
        ...
        if _can_merge_group(group_rows, group_decisions, group_items, fid):
            merged = _merge_family_group(group_items, group_decisions, family, similarities)
