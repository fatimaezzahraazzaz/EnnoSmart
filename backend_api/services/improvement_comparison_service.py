...
def _active_pdf_overlay_for_non_word(
    original_pdf: Path,
    version: ImprovementVersion,
    project_id: int,
) -> tuple[Path, str]:
    """Version active consolidée pour un format figé comme PDF."""
    rows = _changes(version)
    if not rows:
        return original_pdf.resolve(), "active-source"
    ...
    target = (
        ...
        / str(project_id)
        / str(version.id)
        / f"active_{_hash(original_pdf, stat.st_mtime_ns, version.id)}.pdf"
    )

    if _valid_pdf(target):
        return target.resolve(), "active-review-overlay"
    ...
    try:
        for change in rows:
            ...
            if matched and rectangles:
                if before and not after:
                    _mark_red(page, rectangles)
                    _add_label(page, "SUPPRESSION ACCEPTÉE", RED, RED_FILL)
                else:
                    _mark_green(page, rectangles)

            if after:
                _green_overlay_note(page, after)
                _add_label(page, "VERSION ACTIVE", GREEN, GREEN_FILL)
...
def build_version_document_preview(
    db: Session,
    *,
    project_id: int,
    session_id: str,
    ...
    """Rend la version active même sans proposition candidate."""
    _ensure_root()
    ...
    if not session.source_document_id:
        raise HTTPException(
            status_code=400,
            detail="Cette conversation n'est pas rattachée à un document source.",
        )
    ...
    if version.status == "original" or not rows:
        return ComparisonPreview(
            path=original_pdf.resolve(),
            page=1,
            matched=True,
            mode="active-original",
        )

    if source.suffix.lower() in DOCX_EXTENSIONS:
        active_pdf, mode, _manifest = _base_proposed_pdf(
            source,
            original_pdf,
            version,
            project_id,
        ...
        return ComparisonPreview(
            path=active_pdf.resolve(),
            page=1,
            matched=True,
            mode=f"active-{mode}",
        )

    active_pdf, mode = _active_pdf_overlay_for_non_word(
        original_pdf,
        version,
        project_id,
    )
