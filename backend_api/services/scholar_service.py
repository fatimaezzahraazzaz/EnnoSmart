...
def _build_enriched_scientific_profile(
    ...
    """
    Enrichissement générique pour EnnoScholar.

    Ne contient aucun profil métier codé en dur. Le titre, le texte scientifique
    et les requêtes sont construits depuis :
    - le titre validé par le consultant ;
    - les passages sources ;
    - le domaine détecté si disponible.
    """
    ...
    combined = "\n".join([enriched_title, clean_src, domain_label])
    terms = _top_scientific_terms(combined, max_terms=14)

    suggested_queries: List[str] = []
    if terms:
        suggested_queries.append(" ".join(terms[:8]))
        if len(terms) >= 10:
            suggested_queries.append(" ".join(terms[4:12]))
    if domain_label and terms:
        suggested_queries.append((domain_label + " " + " ".join(terms[:6])).strip())

    context_bits = []
    if domain_label:
        context_bits.append(f"Domaine détecté : {domain_label}.")
    if project_name:
        context_bits.append(f"Projet : {project_name}.")

    scientific_text = (
        f"{enriched_title}. "
        "Verrou scientifique à investiguer à partir des preuves techniques sélectionnées par le consultant. "
        + " ".join(context_bits)
        + f" Termes techniques extraits : {', '.join(terms[:12]) if terms else 'à préciser'}. "
