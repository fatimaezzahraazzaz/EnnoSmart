...
# -*- coding: utf-8 -*-
"""
Test batch avec classification anti-bot / paywall.
Utilise la même logique que le service d'extraction pour classifier chaque URL.
Usage : python test_classify_batch.py
"""
...
# Liste des URLs à tester (avec leur identifiant)
URLS = [
...
def classify_content(html: str, article_title: str = "") -> Dict[str, str]:
    """
    Utilise la fonction de classification du service d'extraction.
    """
    # Cas spéciaux : contenu vide
    if not html or len(html) < 50:
        return {"status": "empty_content", "message": "Contenu vide ou trop court"}

    # Détection rapide des pages de challenge (fallback)
    if _is_antibot_html(html):
        return {"status": "antibot_blocked", "message": "Page de challenge anti-bot détectée"}
    if _is_probably_paywall_or_login(html):
        return {"status": "paywall_blocked", "message": "Paywall / login détecté"}

    # Classification détaillée via le service
    return _classify_html_content(html, article_title)


def test_url(article_id: str, url: str) -> Dict[str, Any]:
    """Teste une URL avec Playwright et retourne les résultats avec classification."""
    print(f"\n{'='*70}")
        ...
        if ok and content:
            ...
            if content[:5] == b"%PDF-":
                ...
                try:
                    ...
                    # On passe un titre vide, la classification utilisera des heuristiques générales
                    classification = classify_content(html, "")
                    ...
                    result["classification_message"] = f"Erreur de décodage : {e}"
...
def main():
    print("\n" + "="*70)
    print("🚀 TEST BATCH AVEC CLASSIFICATION ANTI-BOT / PAYWALL")
    ...
    for i, (article_id, url) in enumerate(URLS, 1):
        ...
        # Pause pour ne pas être trop agressif
        time.sleep(0.5)
    ...
    report_path = Path(__file__).parent / "classification_results.json"
