...
"""
EnnoSmart — Source Highlight V173
=================================
...
OFFICE_CONVERT_TIMEOUT_SECONDS = int(
    os.getenv("ENNOSMART_OFFICE_CONVERT_TIMEOUT", "420")
)
...
# V173 — les deux panneaux du comparatif peuvent demander le même DOCX
# presque simultanément (et React Strict Mode peut aussi doubler l'effet en dev).
# Une seule conversion LibreOffice du même fichier doit donc être autorisée.
OFFICE_CONVERT_LOCK_WAIT_SECONDS = int(
    os.getenv(
        "ENNOSMART_OFFICE_CONVERT_LOCK_WAIT",
        str(OFFICE_CONVERT_TIMEOUT_SECONDS + 120),
    )
)
OFFICE_CONVERT_LOCK_STALE_SECONDS = int(
    os.getenv(
        "ENNOSMART_OFFICE_CONVERT_LOCK_STALE",
        str(max(OFFICE_CONVERT_TIMEOUT_SECONDS * 2, 900)),
    )
)
OFFICE_CONVERT_LOCK_POLL_SECONDS = float(
    os.getenv("ENNOSMART_OFFICE_CONVERT_LOCK_POLL", "0.25")
)
...
def _clean_subprocess_environment_for_libreoffice(
    profile_dir: Path,
) -> Dict[str, str]:
    """
    Construit un environnement propre pour LibreOffice.

    Sur Windows, un backend lancé depuis un venv Python peut hériter de
    PYTHONHOME / PYTHONPATH / VIRTUAL_ENV. Certaines installations de
    LibreOffice interprètent alors ces variables et affichent :

        Could not find platform independent libraries <prefix>

    LibreOffice n'a pas besoin des variables Python du venv EnnoSmart.
    """
    env = dict(os.environ)
    ...
    # Ne pas forcer HOME vers le dossier de cache EnnoSmart.
    # Le profil LibreOffice est déjà isolé avec UserInstallation.
    if os.name != "nt":
...
def _find_converted_pdf(
    ...
    # LibreOffice peut légèrement normaliser le nom de sortie.
    # Priorité au nom le plus proche, sinon au PDF le plus récent.
    wanted = normalize_filename(source.name)
    ...
    for candidate in candidates:
        ...
        if normalized == wanted:
            score = 100
        elif normalized in wanted or wanted in normalized:
            score = 90
...
def _valid_cached_pdf(path: Path) -> bool:
    """Retourne True uniquement pour un cache PDF réellement exploitable."""
    try:
...
def _conversion_lock_path(target: Path) -> Path:
    return target.with_suffix(".convert.lock")


def _try_acquire_conversion_lock(lock_path: Path) -> bool:
    """
    Lock fichier atomique, valable entre threads ET entre workers/processus.
    O_CREAT|O_EXCL garantit qu'un seul appel devient propriétaire.
    """
...
def _lock_is_stale(lock_path: Path) -> bool:
    try:
        ...
        return age > OFFICE_CONVERT_LOCK_STALE_SECONDS
    except Exception:
        return False


def _acquire_or_wait_for_conversion(
    target: Path,
) -> tuple[bool, Path | None]:
    """
    Retourne :
      (True, None)  -> cet appel doit effectuer la conversion.
      (False, pdf)  -> un autre appel a fini ; réutiliser son cache.

    Ainsi original/proposed ne lancent plus deux soffice.exe lourds en parallèle.
    """
    lock_path = _conversion_lock_path(target)
    deadline = time.monotonic() + max(30, OFFICE_CONVERT_LOCK_WAIT_SECONDS)

    while True:
        if OFFICE_PDF_CACHE_ENABLED and _valid_cached_pdf(target):
            return False, target.resolve()

        if _try_acquire_conversion_lock(lock_path):
            # Re-vérification après acquisition : un autre processus peut avoir
            # publié le cache juste avant que nous prenions le verrou.
            if OFFICE_PDF_CACHE_ENABLED and _valid_cached_pdf(target):
                try:
                    lock_path.unlink()
                except Exception:
        ...
        if time.monotonic() >= deadline:
            raise RuntimeError(
                "Conversion Office -> PDF toujours en cours après "
                f"{OFFICE_CONVERT_LOCK_WAIT_SECONDS}s d'attente."
            )

        time.sleep(max(0.05, OFFICE_CONVERT_LOCK_POLL_SECONDS))
...
def convert_office_to_pdf(
    ...
    V173 - correction Windows robuste + anti-conversion concurrente :
    - sortie LibreOffice dans un dossier temporaire isolé ;
    - profil LibreOffice temporaire indépendant ;
    - suppression de PYTHONHOME / PYTHONPATH hérités du venv ;
    - aucune conversion directement dans le dossier final de cache ;
    - remplacement atomique du PDF de cache après succès ;
    - nettoyage des anciens fichiers PDF nuls/corrompus.

    Cette stratégie évite notamment :
      * "Could not find platform independent libraries <prefix>"
      * "SfxBaseModel::impl_store ... failed: 0x11b"
      * les collisions lorsque plusieurs previews sont demandées.
    """
    ...
    if not source.exists() or not source.is_file():
        raise RuntimeError(
            f"Document Office introuvable : {source}"
        )
    ...
    # V173 : réutiliser le cache ou attendre la conversion identique déjà
    # lancée par l'autre panneau du comparatif.
    conversion_owner, shared_pdf = _acquire_or_wait_for_conversion(
        target
    )
    ...
    if not libreoffice:
        try:
            if conversion_owner and lock_path.exists():
                lock_path.unlink()
        except Exception:
            pass
        raise RuntimeError(
    ...
    # Supprimer un ancien placeholder / PDF nul avant toute tentative.
    try:
    ...
    # IMPORTANT :
    # La conversion se fait dans %TEMP% et NON directement dans
    # storage/previews/... Cela évite les erreurs I/O Windows / fichiers
    # éventuellement utilisés par FastAPI ou le navigateur.
    conversion_root = Path(
        tempfile.mkdtemp(
            prefix="ennosmart_lo_convert_"
        )
    )
    output_dir = conversion_root / "output"
    profile_dir = conversion_root / "profile"
    input_dir = conversion_root / "input"
    ...
    # Utiliser une copie locale temporaire.
    # Cela évite les problèmes de lock OneDrive/SharePoint et garantit
    # que LibreOffice lit un chemin local contrôlé.
    safe_input_name = _safe_filename(
        source.name,
        fallback=f"document{suffix}",
    )
    temp_input = input_dir / safe_input_name
    ...
    try:
        ...
        raise RuntimeError(
            "Impossible de préparer le document Office pour conversion : "
            + str(exc)
        ) from exc
    ...
    try:
        ...
        env = _clean_subprocess_environment_for_libreoffice(
            profile_dir
        )
        ...
        # Vérification minimale du PDF avant de le publier dans le cache.
        try:
            header = produced.read_bytes()[:5]
        except Exception:
            header = b""

        if header != b"%PDF-":
            raise RuntimeError(
                "LibreOffice a produit un fichier qui n'est pas un PDF valide."
            )

        # Publier par fichier temporaire puis os.replace :
        # le cache final n'est jamais partiellement écrit.
        staging_target = target.with_suffix(
            ".staging.pdf"
        )
        ...
        # Si un ancien cache final est présent mais invalide,
        # tenter de le retirer avant le remplacement atomique.
        try:
            if target.exists():
                target.unlink()
        except PermissionError:
            # Rare cas Windows : un ancien fichier peut être verrouillé.
            # Écrire sous un nom unique et l'utiliser directement.
            unlocked_target = (
                target.parent
                / (
                    f"{target.stem}_"
                    f"{short_hash(source, source.stat().st_mtime_ns)}"
                    ".pdf"
                )
            )

            if unlocked_target.exists():
                try:
                    unlocked_target.unlink()
                except Exception:
                    pass
        ...
        metadata = {
            ...
            "stdout": (completed.stdout or "")[-3_000:],
            "stderr": (completed.stderr or "")[-3_000:],
        }
        ...
        raise RuntimeError(
            "Conversion Office -> PDF expirée après "
            f"{OFFICE_CONVERT_TIMEOUT_SECONDS}s."
        ) from exc
        ...
        # Toujours libérer le verrou : succès, erreur ou timeout.
        try:
            if conversion_owner and lock_path.exists():
                lock_path.unlink()
        except Exception:
...
def inline_file_response(
    ...
    *,
    ...
    headers = {
        ...
        # Le frontend de Comparaison docs doit pouvoir lire la page trouvée
        # même lorsque Next.js et FastAPI sont servis sur des origines différentes.
        # Sans expose-headers, le PDF est bien surligné mais le navigateur ne
        # permet pas à fetch() de lire X-EnnoSmart-Highlight-Page.
        "Access-Control-Expose-Headers": (
            "X-EnnoSmart-Highlight-Page, "
            "X-EnnoSmart-Highlight-Exact, "
            "X-EnnoSmart-Highlight-Count, "
            "X-EnnoSmart-Source-File, "
            "X-EnnoSmart-Rendered-File, "
...
def source_highlight_health(
    ...
    return {
        "ok": True,
        "version": "V172-office-pdf-auto-scroll",
