...
function activeVersionPreviewUrl(
  projectId: number,
  sessionId: string,
  activeVersionId: string,
) {
  return (
    `${API_BASE_URL}/api/projects/${projectId}/improvements/` +
    `sessions/${encodeURIComponent(sessionId)}/versions/` +
    `${encodeURIComponent(activeVersionId)}/document-preview`
  )
}
...
function EmptyFuturePane() {
  return (
    <section className="flex min-h-0 min-w-0 flex-1 flex-col overflow-hidden bg-white">
      <div className="flex min-h-12 shrink-0 items-center gap-3 border-b bg-emerald-50/35 px-3 py-2">
        <div className="grid size-8 shrink-0 place-items-center rounded-lg border border-emerald-100 bg-white text-emerald-500/60">
          <FileText className="size-4" />
        </div>
        <div className="min-w-0 flex-1">
          ...
          <p className="truncate text-[11px] text-muted-foreground">
            Aucune amélioration en attente
          </p>
        </div>
      </div>
      <div className="grid min-h-0 flex-1 place-items-center bg-muted/10 p-6">
        <div className="max-w-sm text-center">
          <FileDiff className="mx-auto size-9 text-muted-foreground/35" />
          <p className="mt-3 text-sm font-semibold text-foreground">
            En attente d’une nouvelle amélioration
          </p>
          <p className="mt-1.5 text-xs leading-5 text-muted-foreground">
            La prochaine proposition apparaîtra ici. La version active reste conservée à gauche.
          </p>
        </div>
      </div>
    </section>
...
export function ImprovementPdfComparator({
  ...
  const hasPendingProposal = Boolean(versionId && rows.length > 0)

  const [splitPercent, setSplitPercent] = useState(50)
  const [changeListOpen, setChangeListOpen] = useState(true)
  ...
  useEffect(() => {
    ...
    if (hasPendingProposal || !projectId || !sessionId || !activeVersionId) {
      setActivePreview(EMPTY_PREVIEW)
    ...
    void (async () => {
      try {
        const response = await fetch(
          activeVersionPreviewUrl(projectId, sessionId, activeVersionId),
        ...
        const mode = String(response.headers.get("X-EnnoSmart-Comparison-Mode") || "")
        ...
        setActivePreview({
          ...EMPTY_PREVIEW,
          error: error instanceof Error ? error.message : "Version active indisponible.",
        })
      }
    })()
  ...
  }, [activeVersionId, hasPendingProposal, projectId, sessionId])
  ...
  return (
    <div className="flex h-full min-h-0 flex-col overflow-hidden">
      <div className="shrink-0 border-b bg-card px-3 py-2">
        <div className="flex flex-wrap items-center gap-2">
          ...
          <div className="ml-auto flex items-center gap-2">
            ...
            <button
              type="button"
              onClick={() => setChangeListOpen((value) => !value)}
              className="inline-flex h-8 items-center gap-1.5 rounded-lg border bg-background px-2.5 text-[10px] font-semibold text-foreground transition hover:bg-muted"
              title={changeListOpen ? "Masquer la liste des changements" : "Afficher la liste des changements"}
            >
              {changeListOpen ? (
              ...
              <span className="hidden sm:inline">
                {changeListOpen ? "Masquer changements" : "Changements"}
              </span>
            </button>
            ) : null}
          </div>
      ...
      <div className="flex min-h-0 flex-1 overflow-hidden">
        <aside
          className={cn(
            "min-h-0 shrink-0 overflow-hidden border-r bg-muted/15 transition-[width] duration-200",
            !hasPendingProposal || changeListOpen
              ? "w-[220px] 2xl:w-[240px]"
              : "w-[42px]",
          )}
        >
          {hasPendingProposal && !changeListOpen ? (
            <div className="flex h-full flex-col items-center gap-2 py-2">
              <button
                ...
                className="grid size-8 place-items-center rounded-lg border bg-background text-muted-foreground hover:bg-muted hover:text-foreground"
                title="Afficher les changements"
                aria-label="Afficher les changements"
              >
          ...
          <div className="space-y-1.5 pb-3">
            {!hasPendingProposal ? (
              <div className="rounded-xl border border-dashed bg-background/60 px-3 py-4 text-center">
                <p className="text-[11px] font-semibold text-foreground">Aucune modification en attente</p>
                <p className="mt-1 text-[10px] leading-4 text-muted-foreground">
                  Cette zone restera vide jusqu’à la prochaine amélioration.
                </p>
              </div>
            ...
            >
              {hasPendingProposal ? (
                ...
                <PdfPane
                  ...
                  reloadToken={`${activeVersionId || "none"}:active`}
                />
              )}
            </div>
