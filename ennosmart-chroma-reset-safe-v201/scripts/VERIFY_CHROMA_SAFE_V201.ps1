...
Write-Host "=== Verification V201 ===" -ForegroundColor Cyan
...
$Checks = @(
    @("version V201", $Text.Contains("diagnostic_generated_artifacts_reset_v2_chroma_safe")),
    @("chunks.json seul nettoye", $Text.Contains('Path(project_store.rag_dir) / "chunks.json"')),
...
if ($Failed) {
    throw "Verification V201 echouee."
}

Write-Host ""
Write-Host "V201 OK." -ForegroundColor Green
