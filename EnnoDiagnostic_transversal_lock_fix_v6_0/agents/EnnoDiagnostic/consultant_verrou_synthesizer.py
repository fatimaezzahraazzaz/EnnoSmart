# -*- coding: utf-8 -*-
# ENNODIAG_FULL_FIX_V5_20260829 — strict NLP authority + false-lock rejection
from __future__ import annotations

# ENNODIAG_FINAL_FIX_V4_20260829 — consultant_verrou_synthesizer
...
VERSION = "v192_nlp_authority_explicit_lock_preservation_title_salvage"
...
def _nlp_group_report_from_sections(
    ...
    """Projette les groupes NLP officiels sans inventaire transversal aval.

    Principe V5 :
    - les groupes déjà produits par le NLP restent l'autorité de composition ;
    - EnnoDiagnostic peut ajouter un *candidat de récupération* uniquement s'il
      porte explicitement ``recovered_missing_lock_candidate=True`` ;
    - une limite, une méthode, un titre, une RQ, un tableau ou une phrase de
      transcription ne devient jamais un verrou simplement parce qu'elle se
    ...
    for source in preferred or []:
        ...
        source_type = _norm(meta.get("source_type") or source.get("source_type"))
        recovered = bool(
            meta.get("recovered_missing_lock_candidate")
            or source.get("recovered_missing_lock_candidate")
        )
        # Seuls les groupes lus directement depuis nlp_result.json sont l'autorité
        # de composition. Les chunks Chroma role=verrou restent des preuves, pas
        # des groupes supplémentaires.
        official_group = source_type == "nlp_result_direct_group"
        if official_group or recovered:
            raw_sources.append(source)
    ...
    for source in raw_sources:
        ...
        if not group_id:
            group_id = ("recovered_constraint_" if recovered else "nlp_group_") + hashlib.sha1(
                f"{meta.get('document')}|{_source_text(source)}".encode("utf-8", errors="ignore")
            ).hexdigest()[:16]
        groups.setdefault(group_id, []).append(source)

    def group_rejection_reason(group_sources: Sequence[Mapping[str, Any]]) -> str:
        """Écarte les faux groupes évidents sans reclasser les vrais verrous.

        Les règles portent sur la fonction documentaire : littérature tierce,
        tableau brut, méthode/outil sans incertitude, ou libellé de structure.
        Un titre/texte explicitement situé sous une section Verrou reste conservé.
        """
        text = " ".join(_source_text(source) for source in group_sources)
        ...
        recovered = any(
            bool(_metadata(source).get("recovered_missing_lock_candidate") or source.get("recovered_missing_lock_candidate"))
            for source in group_sources
        )
        if recovered:
            return ""
        ...
        project_anchor = bool(re.search(
            r"\b(?:nous|notre|nos|l equipe|dans ce projet|dans le cadre du projet|on a|nous avons)\b",
            normalized,
            flags=re.I,
        ))
        external_markers = re.findall(
            r"\b(?:analyse de \d+ (?:papers?|articles?|publications?)|dans le papier|the paper|les auteurs|"
            r"the authors|selon (?:l etude|l article|les auteurs)|publications? recentes?|survey|systematic review|"
            r"l etude|les etudes|et al\.)\b",
            normalized,
            flags=re.I,
        )
        if (len(external_markers) >= 2 or (external_markers and not project_anchor)) and not explicit_lock_section:
            return "external_literature_group"
        table_like = text.count("|") >= 8 or bool(re.search(r"\[tableau\s+\d+", normalized, flags=re.I))
        uncertainty = bool(re.search(
            r"\b(?:verrou|incertitude|non maitr|impossibilite|difficulte|limite|contrainte|"
            r"reste a demontrer|non garanti|insuffisant|performe tres mal|hallucination|"
            r"defi non resolu|probleme non resolu)\w*\b",
            normalized,
            flags=re.I,
        ))
        if table_like and not uncertainty and not explicit_lock_section:
            return "raw_table_not_lock"
        method_markers = len(re.findall(
            r"\b(?:embedding|vectoriel|prompt|configuration|methode|strategie|pipeline|rag|retrieval|"
            r"selection|classement|parametre|architecture)\w*\b",
            normalized,
            flags=re.I,
        ))
        if method_markers >= 2 and not uncertainty and not explicit_lock_section:
            return "method_or_parameter_without_uncertainty"
        title_noise = all(
            bool(re.match(r"^(?:architecture|configuration|modeles?|evaluate|rq\d*|\[section|\[tableau)",
                          _norm(_metadata(source).get("section_title") or source.get("section_title")), re.I))
            for source in group_sources
        )
        if title_noise and not uncertainty and not explicit_lock_section:
            return "document_structure_not_lock"
        return ""
    ...
    for group_id, sources in groups.items():
        ...
        recovery_kinds = {
            _clean(_metadata(source).get("recovery_kind"))
            for source in sources
            if _clean(_metadata(source).get("recovery_kind"))
        }
        recovery_confidence = max(
            (_safe_float(_metadata(source).get("recovery_confidence"), 0.0) for source in sources),
            default=0.0,
        )
        ...
        for source in sources:
            ...
            if score <= 0 and isinstance(assessment, Mapping):
                score = _safe_float(
                    assessment.get("eligibility_score") or assessment.get("documentary_coverage"),
                    0.0,
                )
            if score > 0:
                scores.append(score)
            ...
            if isinstance(nested, list):
                passages.extend(dict(row) for row in nested if isinstance(row, Mapping))
        ...
        if not passages:
            for source in sources:
                ...
                passages.append({
                    ...
                    "section_title": _clean(meta.get("section_title") or source.get("section_title")),
                    "role": _clean(meta.get("role") or source.get("role") or "verrou"),
                    ...
                    "reference_like": bool(meta.get("reference_like") or source.get("reference_like")),
                    "text": _source_text(source),
                })
        ...
        representative_text = next((_source_text(source) for source in sources if _source_text(source)), "")
        projected.append({
            "cluster_id": group_id,
            "member_group_ids": [group_id],
            "support_group_ids": [],
            ...
            "lock_scope": "project_structuring_lock",
            "cluster_role": "verrou_a_verifier",
            "cluster_role_confidence": (
                round(recovery_confidence or 0.72, 4) if recovered_group else 1.0
            ),
            "cluster_role_reasons": [
                (
                    "incertitude expérimentale implicite récupérée dans le corpus courant "
                    "et soumise à validation"
                    if "implicit_experimental_uncertainty" in recovery_kinds else
                    "contrainte forte récupérée dans le corpus courant et soumise à validation"
                )
                if recovered_group else
                "groupe principal finalisé par le NLP avant Frascati"
            ],
            "related_cluster_ids": [],
            "group_count": 1,
            ...
            "needs_human_validation": True,
            "not_final_cir": True,
            "recovered_missing_lock_candidate": recovered_group,
            "recovery_kinds": sorted(recovery_kinds),
            "explicit_lock_section": explicit_lock_section,
            "grouping_owner": "nlp_before_frascati_with_strict_missing_lock_recovery",
    ...
    return {
        "ok": True,
        "version": "nlp_group_passthrough_v5_strict_recovery",
        "mode": "nlp_group_passthrough_no_transverse_promotion",
        "groups_count": len(projected),
        "display_clusters_count": len(projected),
        "rejected_false_groups": rejected_groups,
        "rejected_false_groups_count": len(rejected_groups),
        "support_only_clusters_count": 0,
        "clusters": projected,
        "coverage": {
...
def _generate(llm: Any, prompt: str, *, request_name: str, max_tokens: int) -> Any:
    ...
    raw = llm.generate(
        ...
        retries=int(os.getenv("ENNOSMART_DIAG_VERROU_LLM_RETRIES", "0" if str(os.getenv("ENNOSMART_DIAG_VERROU_FAST_REPAIR", "1")).strip().lower() in {"1", "true", "yes", "oui", "on"} else "1")),
        json_mode=True,
...
def _salvage_consultant_title(
    draft_title: Any,
    uncertainty: Any,
    cluster: Mapping[str, Any],
) -> str:
    """Produit un titre consultant sans placeholder et sans nouvel appel LLM.

    On conserve d'abord le titre LLM s'il est techniquement ancré mais seulement
    descriptif, en lui ajoutant une forme d'incertitude. À défaut, on dérive une
    formulation depuis l'incertitude ou le libellé NLP officiel.
    """
    raw = _safe_visible(draft_title)
    if raw:
        normalized = _norm(raw)
        words = normalized.split()
        if (
            4 <= len(words) <= CONSULTANT_TITLE_MAX_WORDS
            and not any(re.search(pattern, normalized) for pattern in TASK_TITLE_PATTERNS)
            and not any(re.search(pattern, normalized) for pattern in RAW_LABEL_PATTERNS)
            and _title_content_tokens(raw) & _title_content_tokens(_cluster_grounding_text(cluster))
        ):
            if any(marker in normalized for marker in CONSULTANT_UNCERTAINTY_MARKERS):
                return _truncate(raw, 220)
            candidate = "Incertitude sur " + raw[:1].lower() + raw[1:].strip(" .:;")
            if len(_norm(candidate).split()) <= CONSULTANT_TITLE_MAX_WORDS:
                return _truncate(candidate, 220)

    derived = _title_from_uncertainty(uncertainty, cluster)
    if derived:
        return derived
    ...
    if (
        label
        and 4 <= len(label_norm.split()) <= 18
        and not any(re.search(pattern, label_norm) for pattern in RAW_LABEL_PATTERNS)
        and not re.match(r"^(?:section|tableau|architecture|configuration|rq\d*)\b", label_norm)
    ):
        return _truncate("Incertitude sur " + label[:1].lower() + label[1:].strip(" .:;"), 220)

    # Dernier recours : extraire une phrase de preuve contenant une limite.
    evidence_title = _title_from_evidence_sentence(cluster)
    if evidence_title:
        return evidence_title
    return "Incertitude technique à préciser avant validation CIR"
...
def _complete_analysis_preserving_llm(
    ...
    if "title" in invalid_fields or not _title_is_valid(result.get("title"), cluster):
        result["title"] = _salvage_consultant_title(
            result.get("title"),
            result.get("scientific_uncertainty"),
            cluster,
        )
...
def synthesize_consultant_verrous(
    ...
    **legacy_arguments: Any,
    ...
    if llm is not None and not fast_repair_mode:
    ...
    for cluster in display_clusters:
        ...
        if _analysis_self_invalidates_lock(analysis) and not bool(cluster.get("explicit_lock_section")):
