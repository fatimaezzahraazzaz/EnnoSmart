# -*- coding: utf-8 -*-
# ENNODIAG_ACTIVE_MEMORY_V7_1_20260830 — active memory fast/stable, parent preservation, objective recovery
    ...
    def generate_diagnostic(self, save: bool = True) -> Dict[str, Any]:
        ...
        if use_axis:
            try:
                ...
                # V7.1 — stabilité inter-run du parent transversal. Si le même
                # ensemble atomique exact a déjà produit un parent validé, une
                # variation LLM ultérieure ne doit pas le faire disparaître.
                # Le cache ne s'applique que si le fingerprint des verrous et de
                # leurs preuves N est strictement identique.
                try:
                    _parent_cache_dir = self.diagnostic_dir / "cache"
                    _parent_cache_dir.mkdir(parents=True, exist_ok=True)
                    _parent_cache_path = (
                        _parent_cache_dir / "transversal_parent_cache_v71.json"
                    )

                    def _stable_atom_payload(item: Dict[str, Any]) -> Dict[str, Any]:
                        _sources = []
                        for _src in item.get("sources") or []:
                            if not isinstance(_src, dict):
                                continue
                            _sources.append({
                                "evidence_id": str(
                                    _src.get("evidence_id")
                                    or _src.get("passage_id")
                                    or ""
                                ),
                                "document": str(_src.get("document") or ""),
                                "excerpt": str(
                                    _src.get("excerpt")
                                    or _src.get("text")
                                    or ""
                        ...
                        return {
                            ...
                            "scientific_lock": str(
                                ...
                                or item.get("scientific_uncertainty")
                                or ""
                            ),
                    ...
                    _parent_fingerprint = sha256_text(json.dumps(
                        [_stable_atom_payload(item) for item in atomic_verrous],
                        ensure_ascii=False,
                        sort_keys=True,
                        default=str,
                    ))

                    if transversal_parent_verrous:
                        try:
                            _parent_cache_path.write_text(
                                json.dumps(
                                    {
                                        "version": "transversal_parent_cache_v71",
                                        "fingerprint": _parent_fingerprint,
                                        "parents": transversal_parent_verrous,
                                    },
                                    ensure_ascii=False,
                                    indent=2,
                                    default=str,
                        ...
                        _cached_parent = safe_read_json(_parent_cache_path)
                        if (
                            _cached_parent.get("version")
                            == "transversal_parent_cache_v71"
                            and _cached_parent.get("fingerprint")
                            == _parent_fingerprint
                            and isinstance(_cached_parent.get("parents"), list)
                        ):
                            _parents = [
                                dict(item)
                                for item in _cached_parent.get("parents") or []
                                if isinstance(item, dict)
                            ]
                            if _parents:
                                transversal_parent_verrous = _parents
                                transversal_selection_audit.append({
                                    "selected": True,
                                    "reason": "stable_parent_reused_same_atomic_fingerprint",
                                    "count": len(_parents),
                                })
                                scientific_axis_report[
                                    "stable_parent_cache_reused"
                                ] = True
                                print(
                                    "[EnnoDiagnostic][SCIENTIFIC_AXIS] "
                                    f"stable_parent_reused={len(_parents)}",
                                    flush=True,
                                )
                except Exception as _parent_cache_exc:
                    scientific_axis_report[
                        "stable_parent_cache_error"
                    ] = str(_parent_cache_exc)

                if not transversal_parent_verrous:
                    _reasons = [
                        str(row.get("reason"))
                        for row in transversal_selection_audit
                        if isinstance(row, dict) and row.get("reason")
                    ]
                    print(
                        "[EnnoDiagnostic][SCIENTIFIC_AXIS] parent_count=0 reasons="
                        + " | ".join(_reasons[:8]),
                        flush=True,
                    )
