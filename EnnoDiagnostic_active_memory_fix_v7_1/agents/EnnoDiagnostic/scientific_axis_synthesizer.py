# -*- coding: utf-8 -*-
# ENNODIAG_ACTIVE_MEMORY_V7_1_20260830 — additive transversal axes + stable parent repair
from __future__ import annotations

# ENNODIAG_FINAL_FIX_V4_20260829 — scientific_axis_synthesizer
...
VERSION = "scientific_axis_synthesizer_v5_5_stable_parent_repair"
...
- Une fusion multi-candidats n'est acceptable que si les preuves courantes montrent
  réellement une incertitude parent transversale expliquant plusieurs sous-problèmes
  simultanément (par exemple interaction de paramètres, conditions aux limites,
  corrélation calcul/mesure, compromis ou dépendances croisées). Ces exemples sont des
  formes génériques de raisonnement, jamais une taxonomie imposée au projet.
- N'invente JAMAIS une incertitude parent absente des preuves. Si les candidats décrivent
  des mécanismes indépendants, conserve des axes singleton. L'axe parent doit être plus
  abstrait que ses membres tout en restant directement ancré dans leurs extraits courants.
...
def _title_is_grounded(title: str, members: Sequence[Mapping[str, Any]]) -> bool:
    ...
    overlap = title_tokens & current_tokens
    ratio = len(overlap) / max(1, len(title_tokens))
    return len(overlap) >= 2 and ratio >= 0.33
...
def _history_for_axis(members: Sequence[Mapping[str, Any]]) -> Dict[str, Any]:
    ...
    historical_objectives: List[Dict[str, Any]] = []
    historical_locks: List[Dict[str, Any]] = []
    current_support: Dict[str, List[Dict[str, Any]]] = {}
    seen_story: Set[Tuple[str, str]] = set()
    for history in histories:
        support_current = history.get("current_support")
        support_current = support_current if isinstance(support_current, Mapping) else {}
        for role, rows in support_current.items():
            if not isinstance(rows, list):
                continue
            current_support.setdefault(str(role), []).extend(
                deepcopy(dict(row)) for row in rows if isinstance(row, Mapping)
            )
        ...
        for role, target in (
            ...
            ("objectif", historical_objectives),
        ):
    ...
    return {
        ...
        "historical_objective_context": historical_objectives[:12],
        ...
        "historical_objective_context_count": len(historical_objectives),
        "current_support": {
            role: _dedupe_dicts(rows, ("evidence_id", "document", "text"))[:12]
            for role, rows in current_support.items()
        },
        "current_support_is_current_proof": True,
        "integrated_with_current_lock_card": bool(continuity),
        "history_is_current_proof": False,
        "usage": (
            "integrated_n_minus_1_lock_method_parameter_result_objective_context; "
...
def _repair_missing_transversal_parent_with_llm(
    llm: Any,
    current: Sequence[Mapping[str, Any]],
) -> Dict[str, Any]:
    """Deuxième chance ciblée si le premier passage ne produit aucun parent.

    Ce retry ne sert jamais à forcer une fusion. Il est déclenché uniquement si
    plusieurs verrous atomiques courants possèdent déjà un pont sémantique dans
    leurs propres preuves N. Le LLM doit alors soit proposer un parent ancré, soit
    répondre qu'aucun parent n'est démontré.
    """
    if (
        llm is None
        or len(current) < 2
        or not _bool_env("ENNOSMART_SCIENTIFIC_AXIS_PARENT_REPAIR", True)
        or not _members_have_bridge(current)
    ):
        return {"ok": False, "used": False, "reason": "repair_not_applicable"}
    ...
    prompt = f"""
Tu vérifies une seule chose : existe-t-il, dans les preuves COURANTES N,
un verrou scientifique parent réellement commun à au moins deux verrous atomiques ?

Règles :
- Ne fusionne pas par simple proximité de vocabulaire.
- Le parent doit expliquer plusieurs sous-problèmes simultanément.
- Chaque mot technique important du titre, de l'incertitude et du mécanisme
  doit être ancré dans les titres, scientific_lock, why_not_simple_engineering
  ou extraits de preuves N fournis.
- Si aucun parent n'est démontré, retourne {{"axis": null}}.
- N'utilise aucun fait historique et n'invente aucun mécanisme.

VERROUS ATOMIQUES COURANTS :
...
""".strip()
    try:
        ...
        try:
            raw = llm.generate(
                prompt,
                request_name="ennodiagnostic:scientific_axis_parent_repair",
                **kwargs,
            )
        except TypeError:
            kwargs.pop("json_mode", None)
            raw = llm.generate(prompt, **kwargs)
        data = _extract_json_object(raw)
        axis = data.get("axis") if isinstance(data, Mapping) else None
        if not isinstance(axis, Mapping):
            return {"ok": True, "used": True, "axis": None}
        return {"ok": True, "used": True, "axis": dict(axis)}
    except Exception as exc:
...
def synthesize_scientific_axes(
    *,
    ...
    cache_key = hashlib.sha256(
        json.dumps(
            {
                "version": "stable_atomic_axis_cache_v1",
                "current": current,
    ...
    parent_repair_report: Dict[str, Any] = {
        "ok": False,
        "used": False,
        "reason": "initial_parent_available",
    }
    has_multi_parent = any(
        len(axis.get("member_current_ids") or []) >= 2
        for axis in axes
        if isinstance(axis, Mapping)
    )
    if not has_multi_parent and len(current) >= 2:
        parent_repair_report = _repair_missing_transversal_parent_with_llm(
            llm,
            current,
        )
        repaired_axis = (
            parent_repair_report.get("axis")
            if isinstance(parent_repair_report, Mapping)
            else None
        )
        if isinstance(repaired_axis, Mapping):
            repaired_proposal = {
                "axes": [dict(repaired_axis)],
                "contextual_items": [],
            }
            repaired_axes, _, repaired_errors = _validate_and_build(
                current,
                repaired_proposal,
            )
            repaired_multi = [
                axis for axis in repaired_axes
                if len(axis.get("member_current_ids") or []) >= 2
            ]
            if repaired_multi:
                # Le retry ciblé ne remplace pas les singletons déjà valides ;
                # il ajoute seulement le parent démontré.
                seen_keys = {
                    tuple(sorted(axis.get("member_current_ids") or []))
                    for axis in axes
                }
                for axis in repaired_multi:
                    key = tuple(sorted(axis.get("member_current_ids") or []))
                    if key not in seen_keys:
                        axes.append(axis)
                        seen_keys.add(key)
                validation_errors = [
                    err for err in validation_errors
                    if err != "aucun_axe_valide"
                ]
                parent_repair_report["accepted"] = True
                parent_repair_report["validation_errors"] = repaired_errors
            else:
                parent_repair_report["accepted"] = False
                parent_repair_report["validation_errors"] = repaired_errors

    # V5 : l'inventaire transversal sert uniquement d'audit. Il ne peut plus
    # créer un axe absent des candidats NLP/recovery officiels.
    explicit_coverage = []
    explicit_warnings = []
    fatal_errors = [
    ...
    report: Dict[str, Any] = {
        ...
        "policy": {
            ...
            "historical_usage": "active_memory_continuity_gap_and_section_orientation_without_current_fact_inference",
            "fail_open": "keep_reconciled_atomic_candidates",
            "explicit_declared_locks_are_mandatory_coverage": False,
            "llm_may_not_delete_explicit_lock_family": False,
        },
        ...
        "parent_repair_report": parent_repair_report,
        "cache_key": cache_key,
...
def select_transversal_axis_additions(
    *,
    atomic_verrous: Sequence[Mapping[str, Any]],
    scientific_axis_report: Optional[Mapping[str, Any]],
) -> Tuple[List[Dict[str, Any]], List[Dict[str, Any]]]:
    """Sélectionne uniquement les axes parents réellement transversaux.

    Cette fonction est volontairement *additive* :
    - elle ne modifie ni ne supprime jamais un verrou atomique ;
    - un axe singleton n'est jamais ajouté, car il dupliquerait le verrou existant ;
    - un axe parent doit réunir au moins deux candidats courants distincts ;
    - chaque membre doit apporter une preuve courante ;
    - le titre, l'incertitude et le mécanisme commun restent soumis aux gardes de
      grounding déjà utilisées par ``_validate_and_build`` ;
    - aucun nom de projet, domaine, technologie, paramètre ou nombre cible de
      verrous n'est codé en dur.

    Le résultat est une liste de *candidats parents à valider par le consultant*.
    Les cartes atomiques restent l'autorité documentaire et Frascati.
    """
    atoms = [
        deepcopy(dict(item))
        for item in (atomic_verrous or [])
        if isinstance(item, Mapping)
    ]
    report = scientific_axis_report if isinstance(scientific_axis_report, Mapping) else {}
    axes = [
        dict(axis)
        for axis in (report.get("scientific_axes") or [])
        if isinstance(axis, Mapping)
    ]

    by_id = {
        _item_id(item, index): item
        for index, item in enumerate(atoms, start=1)
    }
    ...
    for axis in axes:
        axis_id = _clean(axis.get("axis_id") or axis.get("group_id"), 120)
        member_ids = list(dict.fromkeys(
            _clean(value, 80)
            for value in (axis.get("member_current_ids") or [])
            if _clean(value, 80) in by_id
        ))
        ...
        def reject(reason: str) -> None:
            audit.append({
                "axis_id": axis_id,
                "member_current_ids": member_ids,
                "selected": False,
                "reason": reason,
            })

        # Un singleton est déjà représenté par sa carte atomique.
        if len(member_ids) < 2:
            reject("singleton_or_unresolved_members")
            continue

        if member_key in seen_member_sets:
            reject("duplicate_member_set")
            continue

        members = [by_id[value] for value in member_ids]
        title = _clean(axis.get("title") or axis.get("technical_axis"), 320)
        uncertainty = _clean(axis.get("scientific_lock") or axis.get("text"), 1000)
        mechanism = _clean(axis.get("common_mechanism") or axis.get("justification"), 1000)
        confidence = _float(axis.get("confidence"))

        # Même seuil que la validation amont : aucune nouvelle permissivité.
        if confidence < 0.66:
            reject("confidence_below_existing_axis_guard")
            continue

        if len(title) < 14 or len(uncertainty) < 20 or len(mechanism) < 20:
            reject("missing_grounded_parent_formulation")
            continue

        if (
            _is_generic_meta_lock(axis)
            or _is_placeholder_lock(axis)
            or _is_self_invalidating_lock(axis)
            or _is_metric_or_method_only_lock(axis)
            or _is_embedded_use_case_lock(axis)
        ):
            reject("axis_is_context_or_non_lock")
            continue

        if (
            not _title_is_grounded(title, members)
            or not _title_is_grounded(uncertainty, members)
            or not _title_is_grounded(mechanism, members)
        ):
            reject("parent_formulation_not_grounded_in_current_members")
            continue

        if not _members_have_bridge(members):
            reject("members_without_current_semantic_bridge")
            continue

        # Chaque sous-problème doit réellement contribuer une preuve courante.
        members_with_sources = 0
        source_signatures: Set[Tuple[str, str, str]] = set()
        for member in members:
            member_has_source = False
            for source in (member.get("sources") or []):
                if not isinstance(source, Mapping):
                    continue
                excerpt = _clean(source.get("excerpt") or source.get("text"), 1000)
                if not excerpt:
                    continue
                member_has_source = True
                source_signatures.add((
                    _clean(source.get("evidence_id") or source.get("passage_id"), 160),
                    _clean(source.get("document"), 500),
                    _clean(excerpt, 600),
                ))
            if member_has_source:
                members_with_sources += 1

        if members_with_sources < 2 or len(source_signatures) < 2:
            reject("insufficient_independent_current_evidence")
            continue

        # L'axe parent ne remplace jamais les cartes atomiques.
        candidate = deepcopy(dict(axis))
        candidate.update({
            "axis_role": "transversal_scientific_axis_parent",
            "lock_scope": "transversal_scientific_axis_parent",
            "display_as_main_lock": True,
            "display_as_lock": True,
            "cluster_role": "verrou_a_verifier",
            "cluster_role_confidence": round(confidence, 4),
            "cluster_role_reasons": [
                "validated_parent_axis_from_multiple_current_atomic_locks",
                "atomic_locks_preserved_without_replacement",
            ],
            "tag_cir": "Signal R&D transversal candidat — à confirmer",
            "frascati_decision": "verrou_a_verifier",
            "consultant_status": "en_attente",
            "candidate_status": "candidate_to_validate",
            "not_final_cir": True,
            "needs_human_validation": True,
            "transversal_augmentation": True,
            "parent_axis_only": True,
            "atomic_members_preserved": True,
            "project_reasoning": mechanism,
            "why_not_simple_engineering": " ; ".join(list(dict.fromkeys(
                _clean(member.get("why_not_simple_engineering"), 520)
                for member in members
                if _clean(member.get("why_not_simple_engineering"), 520)
            )))[:1400],
            "evidence_summary": mechanism,
            "consultant_check": (
                "Valider que ce mécanisme transversal constitue bien un verrou parent "
                "distinct, sans supprimer les sous-problèmes atomiques déjà détectés."
            ),
            "risk_level": "à évaluer",
            "source": "scientific_axis_transversal_augmentation",
            "qualification_source": "validated_multi_atomic_axis_current_project_only",
            "grouping_policy": "additive_parent_axis_atomic_candidates_preserved",
            "llm_generated": bool((report.get("llm_report") or {}).get("used")),
            "upstream_frascati_score": axis.get("frascati_score"),
        })

        # Compatibilité avec la structure des cartes consultant.
        candidate["member_group_ids"] = list(dict.fromkeys(
            _clean(value, 240)
            for value in (
                list(candidate.get("member_group_ids") or [])
                + list(candidate.get("original_nlp_group_ids") or [])
            )
            if _clean(value, 240)
        ))
        candidate["original_nlp_group_ids"] = list(candidate["member_group_ids"])
        candidate["source_ids"] = list(dict.fromkeys(
            _clean(source.get("evidence_id") or source.get("passage_id"), 160)
            for source in (candidate.get("sources") or [])
            if isinstance(source, Mapping)
            and _clean(source.get("evidence_id") or source.get("passage_id"), 160)
        ))
        candidate["source_json"] = {
            "source": candidate["source"],
            "axis_id": candidate.get("axis_id"),
            "member_current_ids": member_ids,
            "member_group_ids": candidate["member_group_ids"],
            "sources": candidate.get("sources") or [],
            "principle": (
                "Axe parent ajouté uniquement après validation multi-candidats. "
                "Les verrous atomiques, leurs scores Frascati et leurs preuves restent inchangés."
            ),
        }
        ...
        audit.append({
            ...
            "member_current_ids": member_ids,
            "selected": True,
            "reason": "validated_transversal_parent_axis",
            "title": title,
...
__all__ = ["VERSION", "synthesize_scientific_axes", "select_transversal_axis_additions"]
