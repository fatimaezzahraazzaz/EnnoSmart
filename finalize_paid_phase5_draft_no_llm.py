...
def markdown_from_draft(writer: Any, draft: dict[str, Any], guard: dict[str, Any]) -> str:
    fn = getattr(writer, "draft_to_markdown", None)
    if callable(fn):
        try:
            return fn(draft, guard)
        except TypeError:
            return fn(draft)
...
def main() -> int:
    ...
    if actual_titles != expected_titles:
        raise SystemExit(
            "FINALIZE_BLOCKED: les titres ne correspondent pas au plan consultant."
        )
    if not summary["ok"]:
        raise SystemExit(
            "FINALIZE_BLOCKED: le garde réel échoue encore: "
            + json.dumps(summary, ensure_ascii=False, default=str)
        )

    output_dir = ctx["base"] / "phase_5_state_of_art_writer"
    json_output = output_dir / "state_of_art_draft_consultant_validated_no_llm.json"
    md_output = output_dir / "state_of_art_draft_consultant_validated_no_llm.md"

    payload = copy.deepcopy(ctx["draft_payload"])
    replaced = False

    def replace_draft(node: Any) -> bool:
        nonlocal replaced
        if isinstance(node, dict):
            if node is ctx["draft"]:
                return False
            sections = node.get("sections")
            if (
                isinstance(sections, list)
                ...
                node.clear()
                node.update(copy.deepcopy(candidate))
                replaced = True
                return True
            for value in node.values():
                if replace_draft(value):
                    return True
        elif isinstance(node, list):
            for value in node:
                if replace_draft(value):
    ...
    replace_draft(payload)
    if not replaced:
        payload = {
            "ok": True,
            "status": "validated_no_llm",
            "draft_json": candidate,
        }

    if isinstance(payload, dict):
        payload["ok"] = True
        payload["status"] = "validated_no_llm"
        payload["guard"] = report
        payload["consultant_plan_snapshot_path"] = str(ctx["snapshot_path"])
        payload["no_llm_revalidation"] = {
            "llm_calls": 0,
    ...
    md_output.write_text(
        markdown_from_draft(ctx["writer"], candidate, report),
        encoding="utf-8",
    )

    print("=" * 116)
    print("PHASE5_PAID_DRAFT_FINALIZED_NO_LLM")
