...
print("=" * 100)
print("TEST MULTI-DOCUMENT V7.6.6 - SANS APPEL LLM")
print("=" * 100)
print("PYTHON  =", sys.executable)
print("VERSION =", CHAT_SERVICE_VERSION)

mode = service._wants_exhaustive_multi_document_coverage(QUESTION)
print("MODE_EXHAUSTIF =", mode)
...
for key, name in expected.items():
    ...
    for index, source in enumerate(by_document.get(key, []), start=1):
        ...
        print(_clean(source.get("text"))[:500])

print()
print("=" * 100)

if not mode:
    raise SystemExit("FAIL: mode multi-document exhaustif non détecté.")

if len(subquestions) < 4:
    raise SystemExit(
        f"FAIL: seulement {len(subquestions)} sous-question(s), attendu >= 4."
    )
...
if any(not item.get("candidate_found") for item in report):
    missing_candidates = [
        _clean(item.get("document_name"))
        for item in report
        if not item.get("candidate_found")
    ]
    raise SystemExit(
        "FAIL: aucun candidat pour: " + " | ".join(missing_candidates)
    )

print("MULTIDOC_BALANCED_COVERAGE_TEST_OK")
print("=" * 100)
