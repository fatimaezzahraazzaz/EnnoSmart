...
CHAT_SERVICE_VERSION = "diagnostic_rag_chat_v7_6_experimental_context_guard"
...
class DiagnosticRAGChatService:
    ...
    def _expanded_queries(cls, question: str) -> List[str]:
        ...
        additions = {
            ...
            "result": (
                "résultats mesurés précision performance valeur comparaison conclusion expérimentale"
            ),
            "limitation": (
                "limite faiblesse incertitude contrainte insuffisance cause"
            ),
            "lock": (
    ...
    def _coverage_subquestions(cls, question: str) -> List[str]:
        ...
        La décomposition reste volontairement conservative :
        - séparation sur ; ou sur une nouvelle proposition interrogative
          introduite par "et/mais pourquoi|comment|quel(s)|quelle(s)" ;
        - expansion générique des formulations "avec A et avec B",
          "pour A et pour B", etc. afin de garantir une recherche par entité ;
        - aucune connaissance projet ou domaine n'est utilisée.

        Les questions `protocol_contrast` conservent le flux V7.2/V7.3 déjà validé.
        """
        ...
        boundary = re.compile(
            r"\s*;\s*"
            r"|\s*,\s+(?:et|mais)\s+(?=(?:pourquoi|comment|quels?|quelles?|quel|quelle|dans\s+quel(?:le)?s?)\b)"
            r"|\s+\bet\s+(?=(?:pourquoi|comment|quels?|quelles?|quel|quelle)\b)",
            flags=re.I,
        )
        parts = [
            _clean(part).strip(" ,;")
        ...
        for part in parts:
            ...
            if match:
                ...
                # Garde anti-sursegmentation : uniquement de petits groupes nominaux.
                if (
                    entity_a
                    and entity_b
        ...
        # On limite volontairement la fan-out : coût stable et comportement prévisible.
        return deduped[:5] or [clean_question]
    ...
    def _prioritize_experimental_context(
        ...
        """Réordonne seulement lorsque la question donne un ordre expérimental.

        Exemple générique : « dans cette première évaluation » doit préférer
        l'occurrence de résultats la plus ancienne du document, et non un bloc
        ultérieur portant le même titre « Résultats ».

        Aucune source n'est supprimée : on change uniquement l'ordre du catalogue
        envoyé au LLM. Sans qualificatif ordinal explicite, l'ordre V7.5.x reste
        ...
        indexed_result_positions: List[int] = []
        for item in items:
            if not cls._experimental_result_like_source(item):
                continue
            meta = _source_metadata(item)
            idx = _safe_int(meta.get("paragraph_index"))
            if idx >= 0:
                indexed_result_positions.append(idx)

        unique_positions = sorted(set(indexed_result_positions))
        ...
        if stage == "earliest":
            ...
            target = unique_positions[1] if len(unique_positions) >= 2 else unique_positions[0]

        original_order = {id(item): pos for pos, item in enumerate(items)}

        def key(item: Dict[str, Any]) -> tuple:
            meta = _source_metadata(item)
            result_like = cls._experimental_result_like_source(item)
            idx = _safe_int(meta.get("paragraph_index"))
            score = float(item.get("_chat_score") or 0.0)

            if result_like and idx >= 0:
                # Priorité absolue à l'occurrence expérimentale correspondant
                # au qualificatif explicite, puis aux résultats voisins.
                return (
                    0 if idx == target else 1,
                    abs(idx - target),
                    -score,
                    idx,
                )
            if result_like:
                # Un résultat sans position reste devant les passages de contexte,
                # mais derrière une occurrence FullText positionnée.
                return (2, 10_000, -score, 10_000)
            return (3, 10_000, -score, 10_000)
...
14. Utilise le minimum de sources nécessaires, au maximum quatre, sans répétition.
...
""".strip()
    ...
    def _align_and_renumber_citations(
        ...
        cited_order = valid_order[:max_sources]
        ...
        *,
        ...
        if self._question_intent(clean_question) == "protocol_contrast":
            explicit = [
                ...
                if self._protocol_contrast_features(_clean(item.get("text"))).get("explicit_pair")
            ]
            if explicit:
                # V7.3 : une preuve explicit_pair contient déjà les deux côtés
                # du contraste. Elle devient la seule base factuelle du tour.
                # Aucun passage voisin ne peut introduire une autre exception.
                combined_sources = explicit[:2]

        evidence = self._evidence_catalog(combined_sources)
        coverage_plan = self._coverage_plan(coverage_subquestions, evidence)

        if not evidence:
            if resolved_scope:
                scope_name = _clean(resolved_scope.get("document_name")) or "le document sélectionné"
                answer = (
        ...
        raw_answer = self.llm.generate(
            ...
            retries=1,
            json_mode=False,
            request_name="ennodiagnostic:rag_chat_v7_5_3",
        )
        ...
        if resolved_scope:
            ...
            # Après la garde stricte, on renumérote encore une fois pour garantir
            # E1..En sans trou, même si une source a été éliminée.
            strict_mapping = {
            ...
            if strict_mapping:
                def strict_rewrite(match: re.Match[str]) -> str:
                    ...
                    return "[" + ", ".join(dict.fromkeys(clean_ids)) + "]" if clean_ids else ""

                pattern = re.compile(r"\[((?:\s*E\d+\s*[,;]?\s*)+)\]", re.I)
