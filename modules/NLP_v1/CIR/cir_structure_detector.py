...
CIR_STRONG_SIGNALS = [
    'credit impot recherche', 'crédit impôt recherche', 'verrous et incertitudes',
    'demarche experimentale', 'démarche expérimentale', 'etat de l art', "état de l’art",
    'objectifs du projet', 'objectifs vises', 'conclusion et contribution',
    'travaux r&d', 'travaux rd', 'fiche descriptive'
]

SECTION_MARKER_RE = re.compile(r'^\s*\[SECTION\s*:\s*(.+?)\]\s*$', re.I)
NUMBERED_HEADING_RE = re.compile(r'^\s*(\d+(?:\.\d+){0,6})\.?\s+(.{3,180}?)\s*$')
PLAIN_HEADING_RE = re.compile(r'^\s*(FICHE DESCRIPTIVE DU PROJET|TH[ÉE]SAURUS|MOTS[- ]CL[ÉE]S?)\s*$', re.I)
...
def _looks_like_noise_heading(line: str) -> bool:
    ...
    if '|' in l and len(l) > 25:
        return True
    if re.search(r'\b(tableau|figure|fig\.|rev\.|modification|date|nom / name|echelle|scale|planche|sheet)\b', low):
        return True
    if re.search(r'\b(chef s de projet|ressources humaines|prenom|nom prenom)\b', low):
        return True
    # bloque listes simples 1. / 2. qui ne sont pas des titres hiérarchiques CIR
...
def _is_heading(line: str) -> Optional[Dict[str, Any]]:
    ...
    if m:
        ...
        if _looks_like_noise_heading(title):
            return None
        return {'section_id': None, 'title': title, 'level': 1, 'raw_title': raw, 'is_marker': True}
    ...
    if m:
        ...
        if _looks_like_noise_heading(title):
            return None
        # Pour éviter les faux titres de données : il faut au moins 1.1 ou un titre CIR fort pour niveau 1
        if '.' not in sid and not any(k in _norm(title) for k in ['projet','fiche','annexe']):
            return None
        return {'section_id': sid, 'title': title, 'level': _heading_level(sid), 'raw_title': raw, 'is_marker': False}

    m = PLAIN_HEADING_RE.match(raw)
    if m:
        return {'section_id': None, 'title': m.group(1).strip(), 'level': 1, 'raw_title': raw, 'is_marker': False}
...
def _content_is_only_toc(text: str) -> bool:
    ...
    toc_like = 0
    for l in lines[:80]:
        if re.match(r'^\d+(?:\.\d+)*\.?\s+.{3,120}\s+\d{1,3}$', l):
            toc_like += 1
    return toc_like >= max(5, len(lines[:80]) // 3)
...
def detect_cir_structure(text: str, document: str = '') -> Dict[str, Any]:
    ...
    if not sections and '[SECTION' in text:
        parts = re.split(r'(\[SECTION\s*:\s*.+?\])', text, flags=re.I)
        cur = None
        for part in parts:
            ...
            if m:
                if cur:
                    sections.append(cur)
                cur = {'section_id': None, 'title': m.group(1).strip(), 'level': 1, 'raw_title': part.strip(), 'text': ''}
            elif cur:
                cur['text'] += '\n' + part
        if cur:
            sections.append(cur)
    ...
    low = _norm(text[:80000])
    keyword_hits = sum(1 for k in CIR_STRONG_SIGNALS if _norm(k) in low)
    important_heading_hits = sum(1 for s in mapped if s.get('section_type') in {'objectifs','etat_art','verrous','methodes_travaux','contribution'})
    numbered_heading_count = sum(1 for s in mapped if s.get('section_id'))

    is_cir = keyword_hits >= 3 or important_heading_hits >= 4 or (numbered_heading_count >= 8 and keyword_hits >= 2)
