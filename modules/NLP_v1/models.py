...
def run_verrou_detector(items: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    # On score seulement les verrous directs + limites fortes, pour rester rapide.
    target_items = [
        x for x in items
        if x.get("role") in {"verrou", "limite"}
    ]
    ...
    preds = detect_verrous_batch([str(x.get("model_input") or x.get("text", "")) for x in target_items])
