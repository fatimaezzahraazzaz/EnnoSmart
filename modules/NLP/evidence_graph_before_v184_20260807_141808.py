...
VERSION = "lock_evidence_grouping_v183_hybrid_macro_consolidation"
...
def build_technical_lock_groups(
    ...
    *,
    ...
    # ========================================================
    # V183 ? CONSOLIDATION SEMANTIQUE DES MICRO-GROUPES
    # ========================================================
    #
    # Le complete-linkage pr?c?dent est volontairement
    # conserv? comme premi?re ?tape prudente.
    #
    # Son d?faut est toutefois de fragmenter un m?me verrou
    # conceptuel lorsqu'il est exprim? avec plusieurs
    # formulations diff?rentes.
    #
    # Cette seconde ?tape rapproche donc les micro-groupes
    # lorsqu'ils repr?sentent le m?me axe technique.
    #
    # IMPORTANT :
    # - aucun nom de projet ;
    # - aucun verrou connu ;
    # - aucun nombre cible de groupes ;
    # - aucun vocabulaire m?tier sp?cifique cod? en dur.
    # ========================================================

    micro_groups_count = len(clusters)

    consolidation_audit: List[Dict[str, Any]] = []

    # --------------------------------------------------------
    # Fr?quence documentaire des ancres.
    #
    # Une ancre pr?sente partout dans le corpus est peu
    # discriminante et ne doit pas suffire ? fusionner
    # deux verrous.
    # --------------------------------------------------------

    anchor_df: Counter = Counter()

    seed_anchor_sets: List[Set[str]] = []

    for seed in seeds:
        values = _anchors(seed)
        seed_anchor_sets.append(values)
    ...
    common_anchor_limit = max(
        3,
        int(math.ceil(len(seeds) * 0.35)),
    )

    def discriminative_anchors(
        cluster: Sequence[int],
    ) -> Set[str]:

        values: Set[str] = set()

        for seed_index in cluster:

            for token in seed_anchor_sets[seed_index]:

                # Une ancre pr?sente dans plus de 35 % des
                # graines est consid?r?e comme trop g?n?rale.
                if anchor_df[token] <= common_anchor_limit:
                    values.add(token)
    ...
    def cluster_centroid(
        ...
        for seed_index in cluster:
            ...
            for dim in range(dimension):
                centroid[dim] += float(vector[dim])

        divisor = float(len(cluster))
    ...
    def cross_cluster_statistics(
        ...
        # Couverture :
        #
        # contrairement au complete-linkage, on ne demande
        # plus que TOUS les passages soient proches.
        #
        # On demande qu'une majorit? des membres de chaque
        # groupe poss?de au moins un voisin s?mantique
        # dans l'autre groupe.
        # ----------------------------------------------------

        member_similarity_threshold = 0.44

        left_coverage = (
            sum(
                value >= member_similarity_threshold
                for value in best_for_left
            )
            / len(best_for_left)
            if best_for_left
            else 0.0
        ...
        # ----------------------------------------------------
        # Centro?des s?mantiques
        # ----------------------------------------------------

        left_centroid = cluster_centroid(
            left_cluster
        )

        right_centroid = cluster_centroid(
            right_cluster
        )
        ...
        if left_centroid and right_centroid:
            ...
            # Repli lexical si le mod?le d'embeddings
            # n'est pas disponible.

            centroid_similarity = _jaccard(
                discriminative_anchors(left_cluster),
                discriminative_anchors(right_cluster),
            )


        # ----------------------------------------------------
        # Ancres discriminantes
        # ----------------------------------------------------

        left_anchors = discriminative_anchors(
            left_cluster
        )

        right_anchors = discriminative_anchors(
            right_cluster
        )
        ...
        return {
            ...
            "average_best_semantic":
                round(
                    float(average_best_semantic),
                    4,
                ),

            "max_cross_semantic":
                round(
                    float(max_cross_semantic),
                    4,
                ),

            "max_pair_score":
                round(
                    float(max_pair_score),
                    4,
                ),
    ...
    def macro_merge_decision(
        left_cluster: Sequence[int],
        right_cluster: Sequence[int],
    ) -> Tuple[bool, float, Dict[str, Any]]:

        stats = cross_cluster_statistics(
            left_cluster,
            right_cluster,
        )

        centroid = float(
            stats["centroid_similarity"]
        )
        ...
        average_best = float(
            stats["average_best_semantic"]
        )

        max_semantic = float(
            stats["max_cross_semantic"]
        )

        max_pair = float(
            stats["max_pair_score"]
        )

        anchor_overlap = float(
            stats["anchor_overlap"]
        )

        shared_count = int(
            stats["shared_anchor_count"]
        )


        # ----------------------------------------------------
        # Trois chemins g?n?riques de fusion.
        #
        # A. Similarit? s?mantique tr?s forte.
        #
        # B. Similarit? forte + majorit? des passages
        #    reli?s + preuve lexicale ou pont fort.
        #
        # C. Similarit? mod?r?e mais couverture tr?s forte
        #    et plusieurs ancres discriminantes communes.
        #
        # Cela ?vite l'ancien probl?me o? un seul passage
        # l?g?rement diff?rent emp?chait toute fusion.
        # ----------------------------------------------------

        very_strong_semantic = bool(
            centroid >= 0.72
            and coverage >= 0.50
            and max_semantic >= 0.58
        )

        coherent_semantic_family = bool(
            centroid >= 0.60
            and coverage >= 0.55
            and max_semantic >= 0.56
            and (
                shared_count >= 2
                or anchor_overlap >= 0.04
                or max_pair >= 0.58
            )
        )

        strong_multi_member_bridge = bool(
            centroid >= 0.56
            and coverage >= 0.70
            and average_best >= 0.50
            and max_pair >= 0.56
            and shared_count >= 3
        )
        ...
        allowed = bool(
            very_strong_semantic
            or coherent_semantic_family
            or strong_multi_member_bridge
        )


        # Score utilis? uniquement pour choisir la meilleure
        # fusion lorsqu'il existe plusieurs possibilit?s.
        ...
        details = {
            **stats,
            ...
            "coherent_semantic_family":
                coherent_semantic_family,

            "strong_multi_member_bridge":
                strong_multi_member_bridge,
    ...
    # ========================================================
    # AGRE?GATION ITERATIVE DES MICRO-GROUPES
    # ========================================================
    ...
    while True:
        ...
        for left_pos in range(
            ...
            for right_pos in range(
                ...
                allowed, score, details = (
                    macro_merge_decision(
                        clusters[left_pos],
                        clusters[right_pos],
                    )
                )
        ...
        clusters[left_pos].extend(
            clusters[right_pos]
        )

        clusters[left_pos] = sorted(
            set(
                clusters[left_pos]
            )
        )
    ...
    macro_groups_count = len(clusters)
    ...
    return {
        "version": VERSION,
        "method": "seeded_micro_complete_linkage_then_semantic_macro_consolidation_then_support_attachment",
        "candidates_count": len(deduped),
        ...
        "invariants": {
            ...
            "macro_semantic_consolidation": True,
